...
/**
 * Adventist Healthcare Workday RN Job Scraper
 * 
 * CUSTOMIZED FOR ADVENTIST HEALTHCARE:
 * - Employment Type: Extracts from title first, then briefcase icon, then hours
 * - Shift Type: Extracts from title ONLY (not description) to avoid false positives
 * - Removes internal employee text from descriptions
 * - Adjusted bullet point spacing for better readability
 * 
 * This scraper is separate from the base UHS scraper to maintain separation of concerns
 * and avoid breaking working scrapers when customizing for different employers.
 */
class WorkdayAdventistScraper {
  /**
  ...
  async processJob(page, job) {
    ...
    const normalizedJob = {
      ...
      description: this.cleanDescriptionForAdventist(jobDetails.description || job.rawText.substring(0, 500)),
  ...
  /**
  /**
   * Clean description for Adventist Healthcare
   * Removes internal employee text irrelevant to external candidates
   */
  cleanDescriptionForAdventist(description) {
    if (!description) return description;
    
    const textsToRemove = [
      "If you are a current Adventist HealthCare employee, please click this link to apply through your Workday account.",
      "If you are a current Adventist HealthCare employee",
      "please click this link to apply through your Workday account"
    ];
    
    let cleanedDesc = description;
    for (const text of textsToRemove) {
      cleanedDesc = cleanedDesc.replace(new RegExp(text, "gi"), "");
    }
  ...
  async getJobDetails(page, job) {
    ...
    try {
      ...
      const jobDetails = await page.evaluate((selectors) => {
        ...
        // Extract employment type - ADVENTIST-SPECIFIC LOGIC
        // Priority: Title → Briefcase Icon → Hours (different from UHS which prioritizes hours first)
        let employmentType = null;
        
        // *** METHOD 1: Check TITLE FIRST (Adventist often includes employment type in title) ***
        // Examples: "RN, PT Night Shift", "RN, Per Diem", "RN, Full Time Day"
        if (title) {
          const titleLower = title.toLowerCase();
        ...
        if (!employmentType) {
          const employmentTypeSelectors = [
            '[data-automation-id="time"]', // Adventist uses "time" not "timeType"
            '[data-automation-id="timeType"]', // Standard Workday
            '[data-automation-id="jobPostingHeader"] [data-automation-id="time"]',
            '[data-automation-id="jobPostingHeader"] [data-automation-id="timeType"]'
          ];
            ...
            if (employmentElement) {
              ...
              if (employmentText) {
                ...
                if (employmentLower.includes('full time') || employmentLower.includes('full-time')) {
                  ...
                  employmentType = 'Part Time';
                } else if (employmentLower.includes('prn') || employmentLower.includes('per diem')) {
                  employmentType = 'PRN';
        ...
        // *** METHOD 3: Check for explicit PRN/Per Diem mentions in description (last resort) ***
        if (!employmentType) {
          if (bodyText.match(/\b(prn|per diem|per-diem)\s+(position|role|schedule|basis)/i)) {
            employmentType = 'PRN';
          }
        }
        
        // NO HOURS FALLBACK for Adventist - title and briefcase icon are reliable enough
        // If still not found, leave as null (explicit-only approach)

        // Extract shift type - ADVENTIST-SPECIFIC: TITLE ONLY
        // Adventist puts shift in title or doesn't mention it at all
        // Checking description causes false positives
        let shiftType = null;
        
        // ONLY check title for shift information (Adventist-specific approach)
        if (title) {
          const titleLower = title.toLowerCase();
          
          // Check for specific shifts FIRST (night/day/evening)
          // Then check for variable (only if no specific shift found)
          if (titleLower.match(/\bnight\s+shift\b/i) || titleLower.match(/\bnight\b/i)) {
            shiftType = 'nights';
          } else if (titleLower.match(/\bday\s+shift\b/i) || titleLower.match(/\bday\b/i)) {
            shiftType = 'days';
          } else if (titleLower.match(/\bevening\s+shift\b/i) || titleLower.match(/\bevening\b/i)) {
            shiftType = 'evenings';
          } else if (titleLower.match(/\brotating\b/i) || titleLower.match(/\brotational\b/i)) {
            shiftType = 'rotating';
...
module.exports = WorkdayAdventistScraper;
