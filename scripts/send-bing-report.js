...
/**
 * Send weekly Bing SEO report email via Brevo
 *
 * Reads the AI summary already saved by pull-bing-data.js and sends
 * a formatted email with stats, alerts, top pages, and top queries.
 *
 * Usage:
 *   node scripts/send-bing-report.js           # Send report for latest data
 *   node scripts/send-bing-report.js --preview  # Print to console, don't send
 *
 * Cron: 0 13 * * 1 (Monday 8 AM EST, 1 hour after pull-bing-data.js)
 */
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f3f4f6;">
  <div style="max-width: 600px; margin: 0 auto; background: #ffffff;">
    ...
    <!-- Header -->
    <div style="background: linear-gradient(135deg, #c2410c, #ea580c); padding: 24px; text-align: center;">
      <h1 style="color: #ffffff; margin: 0; font-size: 20px; font-weight: 700;">Bing Weekly Report</h1>
      <p style="color: #fed7aa; margin: 8px 0 0; font-size: 14px;">Week ending ${dateStr}</p>
    </div>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="border-bottom: 1px solid #e5e7eb;">
      <tr>
        <td style="text-align: center; padding: 16px 8px;">
          <div style="font-size: 24px; font-weight: 700; color: #ea580c;">${summary.totalClicks}</div>
    ...
    <div style="padding: 0 20px 16px;">
      ...
      ${topPages.slice(0, 20).map((p, i) => {
        ...
        <div style="display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px solid #f3f4f6; font-size: 13px;">
          <a href="${p.page}" style="color: #ea580c; text-decoration: none; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${i + 1}. ${truncated}</a>
    ...
    <div style="padding: 16px 20px; background: #f9fafb; text-align: center; border-top: 1px solid #e5e7eb;">
      <a href="https://intelliresume.net/admin/bing" style="color: #ea580c; text-decoration: none; font-size: 13px; font-weight: 500;">View Full Dashboard</a>
      <p style="margin: 8px 0 0; font-size: 11px; color: #9ca3af;">IntelliResume Bing Monitor</p>
    </div>
...
async function main() {
  ...
  // Find latest summary with data
  const latestSummary = await prisma.bingSiteSummary.findFirst({
    orderBy: { date: 'desc' },
  });

  if (!latestSummary) {
    console.error('No Bing data found. Run pull-bing-data.js first.');
    await prisma.$disconnect();
    process.exit(1);
  }

  const dateStr = latestSummary.date.toISOString().split('T')[0];
  console.log(`Preparing Bing report for week ending ${dateStr}...`);

  const aiReport = latestSummary.aiSummary || 'No AI summary available. Run pull-bing-data.js without --skip-ai.';

  // Fetch alerts, top pages (with lag fallback), top queries
  const [alerts, topQueries] = await Promise.all([
  ...
  // Page data may lag behind — use fallback like the dashboard
  let topPages = await prisma.bingPageMetric.findMany({
    where: { date: latestSummary.date },
    orderBy: { clicks: 'desc' },
    take: 20,
  });
  ...
  if (topPages.length === 0) {
    const fallback = await prisma.bingPageMetric.findFirst({
      orderBy: { date: 'desc' },
      select: { date: true },
    });
    if (fallback) {
      topPages = await prisma.bingPageMetric.findMany({
        where: { date: fallback.date },
        orderBy: { clicks: 'desc' },
        take: 20,
      });
    }
  ...
  // Build email HTML
  const emailHTML = buildReportEmail(dateStr, aiReport, alerts, latestSummary, topPages, topQueries);
  ...
  sendSmtpEmail.subject = `${subjectEmoji}Bing Report ${dateStr}: ${latestSummary.totalClicks} clicks, ${latestSummary.totalImpressions.toLocaleString()} impressions`;
  ...
  try {
    ...
    console.log(`Bing report sent to: ${RECIPIENTS.map(r => r.email).join(', ')}`);
  } catch (error) {
