...
import { migrateToDatabase } from '../resumeUtils';
  ...
  useEffect(() => {
    const performMigration = async () => {
      ...
      try {
        ...
        // Use the centralized migration function (static import, not dynamic,
        // so we share the same module instance and window-level mutex)
        ...
        if (result.success) {
          ...
          toast.error('Resume not synced. Delete a resume or upgrade your plan, then try again.', {
            id: 'migration-toast',
            duration: 8000,
            style: {
              background: '#78350f',
