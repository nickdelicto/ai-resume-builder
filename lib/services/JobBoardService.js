...
const { queueJobUrl, queueJobUrls } = require('./indexNowQueueService');
  ...
  async saveJob(jobData, employer) {
    try {
      ...
      if (existingJob) {
        ...
        // Reactivate if:
        // 1. Job is currently inactive
        // 2. Job was previously approved (wasEverActive)
        // 3. Doesn't need re-classification
        // Note: Skeleton jobs can never get wasEverActive=true because the classifier
        // skips them. So any wasEverActive=true job was approved with complete data.
        ...
        const updatedJob = await prisma.nursingJob.update({
          ...
          data: {
            ...
            workArrangement: jobData.workArrangement || null,
        ...
        // IndexNow: Queue URL for throttled submission (streaming mode)
        queueJobUrl(updatedJob, 'update').catch(err => {
          console.error(`⚠️ IndexNow queue failed for job ${updatedJob.slug}:`, err.message);
        });
      ...
      // IndexNow: Queue URL for throttled submission (streaming mode)
      queueJobUrl(newJob, 'update').catch(err => {
        console.error(`⚠️ IndexNow queue failed for job ${newJob.slug}:`, err.message);
      });
  ...
  async deactivateExpiredJobs() {
    try {
      ...
      if (expiredJobs.length > 0) {
        ...
        // IndexNow: Queue URLs for throttled submission (streaming mode)
        queueJobUrls(jobsWithSlugs, 'delete').catch(err => {
          console.error(`⚠️ IndexNow queue failed for expired jobs:`, err.message);
        });
  ...
  async verifyActiveJobs(foundSourceUrls, employerId) {
    try {
      ...
      if (activeJobs.length > 0) {
        ...
        // IndexNow: Queue URLs for throttled submission (streaming mode)
        queueJobUrls(jobsWithSlugs, 'delete').catch(err => {
          console.error(`⚠️ IndexNow queue failed for verified inactive jobs:`, err.message);
        });
