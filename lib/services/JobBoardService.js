...
class JobBoardService {
  ...
  async saveJob(jobData, employer) {
    try {
      ...
      if (existingJob) {
        ...
        const updatedJob = await prisma.nursingJob.update({
          ...
          data: {
            ...
            // Otherwise keep existing classification status and isActive state
            isActive: shouldReClassify ? false : (existingJob.classifiedAt ? true : false),
        ...
        // Notify IndexNow about the update (fire and forget - don't block on this)
      ...
      // Notify IndexNow about the new job (fire and forget - don't block on this)
