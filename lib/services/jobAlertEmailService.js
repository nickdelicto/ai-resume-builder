...
async function getMatchingJobs(alert) {
  const whereClause = {
    isActive: true,
    specialty: { equals: alert.specialty, mode: 'insensitive' },
    experienceLevel: { not: 'Leadership' } // Exclude leadership positions
  };

  // Add location filters
  if (alert.city && alert.state) {
    whereClause.city = { contains: alert.city, mode: 'insensitive' };
  ...
  <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
    ...
    <div style="padding: 24px;">
      ${jobs.length > 0 ? `
        ...
        <p style="color: #374151; font-size: 16px; line-height: 1.5; margin: 0 0 24px 0;">
          Great news! We found <strong>${jobs.length} ${alert.specialty} RN position${jobs.length === 1 ? '' : 's'}</strong> in ${alert.location}, sorted by highest paying first:
        </p>
    ...
    <div style="background-color: #f9fafb; padding: 24px; border-top: 1px solid #e5e7eb;">
      ...
      <p style="margin: 0; font-size: 12px; color: #9ca3af; text-align: center;">
        IntelliResume • Helping nurses find better opportunities
      </p>
    </div>
 ...
 */
async function sendJobAlertEmail(alertId) {
  try {
    // Fetch alert details
