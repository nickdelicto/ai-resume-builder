...
async function fetchSpecialtySalaryStats(specialtySlug) {
  console.log('[fetchSpecialtySalaryStats] Called with:', specialtySlug);
  console.log('[fetchSpecialtySalaryStats] prisma:', !!prisma);
  console.log('[fetchSpecialtySalaryStats] prisma.nursingJob:', !!prisma?.nursingJob);
  console.log('[fetchSpecialtySalaryStats] prisma constructor:', prisma?.constructor?.name);
  console.log('[fetchSpecialtySalaryStats] prisma keys:', Object.keys(prisma || {}).slice(0, 10));
...
async function fetchEmployerJobTypeJobs(employerSlug, jobTypeSlug, page = 1, limit = 20) {
  ...
  // Build where clause - handle both "per-diem" and "prn" for Per Diem jobs
  const whereClause = {
    employerId: employer.id,
    isActive: true
  };

  // For Per Diem, match both "per-diem" and "prn"
  if (normalizedJobType === 'per-diem') {
    whereClause.OR = [
      { jobType: { equals: 'per-diem', mode: 'insensitive' } },
      { jobType: { equals: 'prn', mode: 'insensitive' } }
    ];
  } else {
