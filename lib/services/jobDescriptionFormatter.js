...
3. If information for a section is NOT available in the original, SKIP that section entirely
...
7. Use emojis only in the Highlights table as shown
...
## 📋 Highlights
- 💰 **Pay:** [Pay range if mentioned, e.g., "$40.70 - $61.05/hour"]
- 📍 **Location:** [City, State]
- ⏰ **Schedule:** [e.g., "Full-time, Day shift" or "Per Diem, Rotating"]
- 🎁 **Sign-On Bonus:** [e.g., "Up to $25,000" - only if mentioned]

(Only include lines where info is available)
...
(Skip this entire section if no benefits mentioned)
...
2. **Highlights**:
   - Pay: Include if salary/hourly rate mentioned. Use exact numbers.
   - Location: Always include (use the data provided).
   - Schedule: Include if shift type or full-time/part-time mentioned.
   - Sign-On Bonus: Only include if explicitly mentioned.
   - Only include bullet points where info is available.
...
5. **Benefits**: Only include if benefits are explicitly listed. Skip entire section otherwise.

6. **About Employer**: Only include if employer description exists. Skip entire section otherwise.
