...
`;
  ...
  // Insurance companies have unique license requirements and remote work details
  const insuranceEmployers = ['centene', 'unitedhealthgroup', 'cigna', 'humana', 'cvs-health', 'anthem', 'elevance'];
  if (insuranceEmployers.includes(employerSlug)) {
    return `
**⚠️ INSURANCE/MANAGED CARE SPECIFIC INSTRUCTIONS (${job.employer?.name}):**

This is an insurance company job. These roles often accept MULTIPLE license types where having ANY ONE of the listed licenses qualifies you.

**CRITICAL - Additional Highlights for Insurance Jobs:**
Insurance care management and utilization review roles have important details that nurses need to know. Extract and add these to the Highlights section IF PRESENT in the source:

- 🏠 **Residency:** [If "must reside in [state/county]" or "no residency required" is mentioned - this is CRITICAL for remote jobs]
- 🗺️ **Service Area:** [If the role covers specific counties, regions, or areas - e.g., "Covers Steuben and surrounding counties"]
- 📋 **Program:** [Insurance program served, e.g., "Kentucky Medicaid", "Medicare Advantage"]
- 👥 **Population:** [Member population focus, e.g., "Adults 18+ with Medicare + Medicaid (dual eligible)", "members with cancer and kidney disease"]
- 💼 **Work Style:** [How work is performed if specified - e.g., "Field-based", "Telephonic and virtual engagement", "Fully remote"]
- 🚗 **Travel:** [If travel percentage mentioned, e.g., "25-35% travel", "local travel required"]

IMPORTANT: Only include these lines if the information is EXPLICITLY stated in the source. Do NOT guess or infer.

**CRITICAL - License/Certification formatting:**
- When the source lists multiple licenses with "or" between them (e.g., "LCSW required or LMHC required or RN required"), you MUST preserve this relationship
- Format as: "One of the following licenses required: LCSW, LMHC, LPC, LMFT, LMHP, or RN"
- Or use bullet format with clear "OR" indication:
...
- Do NOT list them all as individually "required" - that implies ALL are needed when only ONE is needed
- If the source explicitly says "and/or Compact State Licensure", include that detail

**Pay Range:**
- Preserve EXACT pay range if given (e.g., "$27.02 - $48.55 per hour")
- Include both minimum AND maximum values
...
function buildFormattingPrompt(job) {
  ...
  if (job.brand) structuredFields.push(`**Brand/Division:** ${job.brand}`);
...
- 🏷️ **Brand/Division:** [Sub-brand or division name, e.g., "Optum Care Solutions", "WellMed" - OMIT if not mentioned or same as employer name]
...
2. **Highlights**:
   ...
   - Brand/Division: Include if a sub-brand or division is mentioned in the structured data (e.g., "Optum Care Solutions", "WellMed"). This helps job seekers understand which entity within a larger company they'd work for. OMIT if not mentioned or if it's the same as the employer name.
