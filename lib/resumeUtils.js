...
export async function migrateToDatabase(options = {}) {
  ...
  try {
    ...
    console.log(`📊 [${source}] Getting service from ResumeServiceFactory`);
    const factory = await ResumeServiceFactory.getService({ isAuthenticated: true });
    
    console.log(`📊 [${source}] Service factory returned:`, {
      hasService: !!factory.service,
      serviceType: factory.service ? factory.service.constructor.name : 'undefined',
      needsMigration: factory.needsMigration
    });
    
    if (!factory.service) {
      console.error(`📊 [${source}] CRITICAL ERROR: No service returned from factory`);
      throw new Error('DB service not available for migration - No service returned from factory');
    }
    
    if (factory.service.constructor.name !== 'DbResumeService') {
      console.error(`📊 [${source}] CRITICAL ERROR: Wrong service type returned:`, factory.service.constructor.name);
      throw new Error(`DB service not available for migration - Got ${factory.service.constructor.name} instead`);
    }
    
    // Check if service is available
    console.log(`📊 [${source}] Checking if service is available`);
    if (typeof factory.service.isAvailable === 'function') {
      const isAvailable = factory.service.isAvailable();
      console.log(`📊 [${source}] Service availability check:`, isAvailable);
      
      if (!isAvailable) {
        throw new Error('DB service is not available - isAvailable() returned false');
      }
    } else {
      console.warn(`📊 [${source}] Service does not have isAvailable method`);
    }
    
    console.log(`📊 [${source}] About to call saveResume on service`);
    ...
    console.log(`📊 [${source}] Save result:`, {
