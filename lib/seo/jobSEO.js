...
function generateSalaryPageMetaTags(location, locationType, salaryStats, specialty = null) {
  // locationType: 'state', 'city', 'state-specialty', 'city-specialty', or 'specialty'
  // location: state name, "City, State", or specialty name (for 'specialty' type)
  ...
  } else if (locationType === 'specialty' && specialty) {
    // For specialty-only (nationwide): specialty passed separately, location can be "Nationwide"
    const specialtySlug = specialty.toLowerCase().replace(/\s+/g, '-').replace(/\s*&\s*/g, '-');
    canonicalUrl = `${SITE_URL}/jobs/nursing/specialty/${specialtySlug}/salary`;
  } else {
    // State
    const stateSlug = location.toLowerCase();
