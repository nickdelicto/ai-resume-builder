...
function generateExperienceLevelPageMetaTags(level, levelSlug, jobCount, maxHourlyRate = null) {
  ...
  // New Grad gets special description emphasizing residency programs
  const isNewGrad = levelSlug === 'new-grad';
  const description = jobCount > 0
    ? `Find ${jobCount.toLocaleString()} ${level.toLowerCase()} Registered Nurse jobs nationwide.${isNewGrad ? ' Perfect for new graduate nurses and RN residency programs.' : ''} Browse ${level.toLowerCase()} nursing positions at top hospitals with competitive pay. Apply today!`
    : `Browse ${level.toLowerCase()} Registered Nurse jobs across the United States.${isNewGrad ? ' Entry-level positions with training and mentorship.' : ''} Find nursing opportunities at top healthcare employers.`;

  const keywords = isNewGrad
    ? `new grad rn jobs, new graduate nurse jobs, rn residency programs, entry level rn jobs, new grad nursing positions, nursing residency programs`
...
function generateStateExperienceLevelPageMetaTags(stateCode, stateFullName, level, levelSlug, jobCount, maxHourlyRate = null) {
  ...
  const isNewGrad = levelSlug === 'new-grad';
  const description = jobCount > 0
    ? `Find ${jobCount.toLocaleString()} ${level.toLowerCase()} Registered Nurse jobs in ${stateName}.${isNewGrad ? ' RN residency programs and entry-level positions available.' : ''} Apply today!`
    : `Browse ${level.toLowerCase()} Registered Nurse jobs in ${stateName}.${isNewGrad ? ' Entry-level nursing positions with training.' : ''} Find opportunities at top healthcare employers.`;

  const keywords = isNewGrad
    ? `new grad rn jobs ${stateName.toLowerCase()}, new graduate nurse ${stateCode.toLowerCase()}, rn residency programs ${stateName.toLowerCase()}, entry level rn jobs ${stateCode.toLowerCase()}`
 ...
 * Example: 12 New Grad RN Jobs in Cleveland, OH
 */
function generateCityExperienceLevelPageMetaTags(stateCode, stateFullName, city, level, levelSlug, jobCount, maxHourlyRate = null) {
  const salaryText = getSalaryText(maxHourlyRate, `${level}-${city}-${stateCode}`);
  const title = jobCount > 0
    ? `${jobCount.toLocaleString()} ${level} RN Jobs in ${city}, ${stateCode}${salaryText}`
    : `${level} RN Jobs in ${city}, ${stateCode}${salaryText}`;

  const isNewGrad = levelSlug === 'new-grad';
  const description = jobCount > 0
    ? `Find ${jobCount.toLocaleString()} ${level.toLowerCase()} Registered Nurse jobs in ${city}, ${stateCode}.${isNewGrad ? ' New graduate RN positions and residency programs.' : ''} Apply today!`
    : `Browse ${level.toLowerCase()} Registered Nurse jobs in ${city}, ${stateCode}.${isNewGrad ? ' Entry-level nursing positions.' : ''} Find opportunities at top healthcare employers.`;

  const keywords = isNewGrad
    ? `new grad rn jobs ${city.toLowerCase()}, new graduate nurse ${city.toLowerCase()} ${stateCode.toLowerCase()}, rn residency programs ${city.toLowerCase()}`
...
function generateSpecialtyExperienceLevelPageMetaTags(specialty, specialtySlug, level, levelSlug, jobCount, maxHourlyRate = null) {
  ...
  const isNewGrad = levelSlug === 'new-grad';
  const description = jobCount > 0
    ? `Find ${jobCount.toLocaleString()} ${level.toLowerCase()} ${specialty} Registered Nurse jobs nationwide.${isNewGrad ? ' Great for new grads starting a career in ' + specialty + '.' : ''} Apply today!`
    : `Browse ${level.toLowerCase()} ${specialty} Registered Nurse jobs.${isNewGrad ? ' Entry-level ' + specialty + ' nursing positions.' : ''} Find opportunities at top healthcare employers.`;

  const keywords = isNewGrad
    ? `new grad ${specialty.toLowerCase()} rn jobs, new graduate ${specialty.toLowerCase()} nurse, entry level ${specialty.toLowerCase()} nursing, ${specialty.toLowerCase()} rn residency`
...
function generateEmployerExperienceLevelPageMetaTags(employerName, employerSlug, level, levelSlug, jobCount, maxHourlyRate = null) {
  ...
  const isNewGrad = levelSlug === 'new-grad';
  const description = jobCount > 0
    ? `Find ${jobCount} ${employerName} ${level.toLowerCase()} Registered Nurse ${jobCount === 1 ? 'position' : 'positions'}.${isNewGrad ? ' Great RN residency and new grad programs at ' + employerName + '.' : ''} Apply today!`
    : `Looking for ${employerName} ${level.toLowerCase()} Registered Nurse jobs?${isNewGrad ? ' Explore new grad nursing programs at ' + employerName + '.' : ''} Find opportunities with competitive pay and great benefits.`;

  const keywords = isNewGrad
    ? `${employerName.toLowerCase()} new grad rn jobs, ${employerName.toLowerCase()} rn residency, ${employerName.toLowerCase()} new graduate nurse, new grad nurse ${employerName.toLowerCase()}`
