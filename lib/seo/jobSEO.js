 ...
 */
function generateSalaryPageMetaTags(location, locationType, salaryStats) {
  ...
  if (avgHourly && avgAnnual) {
    title = `Average RN Salary in ${location} - ${formatSalary(avgHourly, 'hourly')} & ${formatSalary(avgAnnual, 'annual')} | IntelliResume`;
  } else if (avgHourly) {
    title = `Average RN Salary in ${location} - ${formatSalary(avgHourly, 'hourly')} | IntelliResume`;
  } else if (avgAnnual) {
    title = `Average RN Salary in ${location} - ${formatSalary(avgAnnual, 'annual')} | IntelliResume`;
  } else {
    title = `Average RN Salary in ${location} | IntelliResume`;
  }
  
  // Build description
  let description;
  if (jobCount > 0 && avgHourly && avgAnnual) {
    description = `Average Registered Nurse (RN) salary in ${location}: ${formatSalary(avgHourly, 'hourly')} (${formatSalary(avgAnnual, 'annual')}). See salary ranges by specialty and employer. Based on ${jobCount} job posting${jobCount === 1 ? '' : 's'} with salary data.`;
  } else if (jobCount > 0) {
    description = `Average Registered Nurse (RN) salary data for ${location}. Based on ${jobCount} job posting${jobCount === 1 ? '' : 's'} with salary information.`;
  } else {
    description = `Registered Nurse (RN) salary information for ${location}. Salary data will appear automatically once jobs with salary information are posted.`;
  }
  
  const keywords = [
    `average rn salary ${location.toLowerCase()}`,
    `registered nurse salary ${location.toLowerCase()}`,
    `nursing salary ${location.toLowerCase()}`,
    `rn pay ${location.toLowerCase()}`,
