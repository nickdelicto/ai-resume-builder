...
function generateCityExperienceLevelPageMetaTags(stateCode, stateFullName, city, level, levelSlug, jobCount, maxHourlyRate = null) {
  ...
  const description = isNewGrad
    ? (jobCount > 0
      ? `Find ${jobCount.toLocaleString()} entry level & new grad RN jobs in ${city}, ${stateCode}. Entry-level nursing positions and residency programs. Apply today!`
      : `Browse entry level & new grad Registered Nurse jobs in ${city}, ${stateCode}. Entry-level nursing positions for new graduate RNs.`)
    : (jobCount > 0
      ? `Find ${jobCount.toLocaleString()} ${level.toLowerCase()} Registered Nurse jobs in ${city}, ${stateCode}. Apply today!`
      : `Browse ${level.toLowerCase()} Registered Nurse jobs in ${city}, ${stateCode}. Find opportunities at top healthcare employers.`);
