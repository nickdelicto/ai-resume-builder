...
function generateJobPostingSchema(job) {
  ...
  // Compute salary values for structured data using normalized hourly values when available
  // This prevents annual salaries (e.g. $75,000) from showing as hourly rates
  const hasSalary = !!(job.salaryMinHourly || job.salaryMaxHourly || job.salaryMin || job.salaryMax);
  let schemaSalaryMin, schemaSalaryMax, schemaSalaryUnit;
  if (job.salaryMinHourly || job.salaryMaxHourly) {
    schemaSalaryMin = job.salaryMinHourly;
    schemaSalaryMax = job.salaryMaxHourly;
    schemaSalaryUnit = "HOUR";
  } else if (job.salaryMin || job.salaryMax) {
    schemaSalaryMin = job.salaryMin;
    schemaSalaryMax = job.salaryMax;
    schemaSalaryUnit = job.salaryType === 'annual' ? "YEAR" : "HOUR";
  }
  ...
  const jobPostingSchema = {
    ...
    "baseSalary": hasSalary ? {
      ...
      "value": {
        ...
        ...(schemaSalaryMin && { "minValue": schemaSalaryMin }),
        ...(schemaSalaryMax && { "maxValue": schemaSalaryMax }),
        "unitText": schemaSalaryUnit
      }
    } : undefined,
    ...
    "directApply": true,  // Our page links directly to employer's apply page
    "industry": "Healthcare",
...
function generateJobMetaTags(job) {
  ...
  // Build salary string using normalized hourly values when available
  // This prevents annual salaries (e.g. $75,000) from displaying as "$75000/hr"
  let salaryStr = '';
  if (job.salaryMinHourly || job.salaryMaxHourly) {
    if (job.salaryMinHourly && job.salaryMaxHourly && job.salaryMinHourly !== job.salaryMaxHourly) {
      salaryStr = `$${job.salaryMinHourly}-${job.salaryMaxHourly}/hr`;
    } else if (job.salaryMinHourly && job.salaryMaxHourly) {
      salaryStr = `$${job.salaryMinHourly}/hr`;
    } else if (job.salaryMinHourly) {
      salaryStr = `$${job.salaryMinHourly}+/hr`;
    } else {
      salaryStr = `Up to $${job.salaryMaxHourly}/hr`;
    }
  } else if (job.salaryMin || job.salaryMax) {
    const isAnnual = job.salaryType === 'annual';
    const fmt = (v) => isAnnual ? `${Math.round(v / 1000)}K` : v;
    const unit = isAnnual ? '/yr' : '/hr';
    if (job.salaryMin && job.salaryMax && job.salaryMin !== job.salaryMax) {
      salaryStr = `$${fmt(job.salaryMin)}-${fmt(job.salaryMax)}${unit}`;
    } else if (job.salaryMin && job.salaryMax) {
      salaryStr = `$${fmt(job.salaryMin)}${unit}`;
    } else if (job.salaryMin) {
      salaryStr = `$${fmt(job.salaryMin)}+${unit}`;
    } else {
      salaryStr = `Up to $${fmt(job.salaryMax)}${unit}`;
    }
  }
  ...
  // Build meta title that fits within 60 chars (Google truncates beyond ~55-60)
  // Cascading: try most informative first, drop details until it fits
  // Priority: Title + Employer + Salary > Title + Employer + Location > Title + Salary
  ...
  // Full: Title at Employer | Salary - Location
  if (salaryStr && employer) {
    titleCandidates.push(`${displayTitle} at ${employer} | ${salaryStr} - ${locationText}`);
    titleCandidates.push(`${displayTitle} at ${employer} | ${salaryStr}`);
  }
  // Without salary: Title at Employer - Location
  if (employer) {
    titleCandidates.push(`${displayTitle} at ${employer} - ${locationText}`);
    titleCandidates.push(`${displayTitle} at ${employer}`);
  }
  // Without employer: Title | Salary - Location
  if (salaryStr) {
    titleCandidates.push(`${displayTitle} | ${salaryStr} - ${locationText}`);
    titleCandidates.push(`${displayTitle} | ${salaryStr}`);
  }
  titleCandidates.push(`${displayTitle} - ${locationText}`);
  titleCandidates.push(displayTitle);

  const title = titleCandidates.find(c => c.length <= 60) || displayTitle.substring(0, 57) + '...';

  // Build compelling, human-readable description (Google shows ~155-160 chars)
  const employerDesc = job.employer?.name || 'A top healthcare employer';
  const locationDesc = job.workArrangement === 'remote'
    ? `Remote${job.state ? ` (${job.state})` : ''}`
    : `${job.city}, ${job.state}`;
  const descSentences = [];
  descSentences.push(`${employerDesc} is hiring: ${displayTitle} in ${locationDesc}.`);
  if (salaryStr) descSentences.push(`${salaryStr}.`);
  const jobDetails = [];
  if (jobTypeStr) jobDetails.push(jobTypeStr);
  if (shiftStr) jobDetails.push(`${shiftStr} shift`);
  if (isNewGrad) jobDetails.push('new grad friendly');
  if (jobDetails.length > 0) descSentences.push(jobDetails.join(', ') + '.');
  descSentences.push('Direct employer listing \u2014 apply today.');

  const description = job.metaDescription || descSentences.join(' ');
 ...
 */
function generateCitySpecialtyPageMetaTags(stateCode, stateFullName, cityName, specialty, stats, maxHourlyRate = null) {
  const stateName = stateFullName || stateCode;
  const jobCount = stats?.total || 0;
  const cityDisplay = getCityDisplayName(cityName, stateCode);
  const salaryText = getSalaryText(maxHourlyRate, `${cityName}-${specialty}`);

  const title = `${jobCount > 0 ? jobCount + ' ' : ''}${specialty} RN Jobs in ${cityDisplay}${salaryText}`;
  const description = jobCount > 0
    ? `${jobCount} ${specialty} Registered Nurse (RN) jobs in ${cityDisplay}. View salaries, benefits, and apply directly. Top hospitals hiring ${specialty.toLowerCase()} nurses in ${cityDisplay} now!`
    : `${specialty} Registered Nurse (RN) jobs in ${cityDisplay}. Positions updated daily. Browse ${specialty.toLowerCase()} nursing opportunities at top hospitals in ${cityDisplay}.`;
