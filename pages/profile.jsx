        ...
        {paymentSuccessResume ? (
          <div style={{
            ...
            <div style={{ 
              ...
              <a 
                href={isEligibleForDownload ? `/api/resume/direct-download?id=${paymentSuccessResume.id}` : '#'} 
                onClick={(e) => {
                ...
                textDecoration: 'none',
                padding: '8px 16px',
                background: isEligibleForDownload ? '#1a73e8' : '#f39c12',
                color: 'white',
                borderRadius: '4px',
                fontSize: '14px',
                display: 'inline-flex',
              ...
              >
                {downloadingResumeId === paymentSuccessResume.id ? (
                  ...
                  <>
                    {eligibleResumeIds.includes(paymentSuccessResume.id) ? (
                      <>
  ...
  const checkResumeEligibility = async () => {
    ...
    try {
      ...
      if (eligibilityData.plan === 'one-time' || subscriptionDetails.planName === 'One-Time Download') {
        ...
        // For other plan types, all resumes are eligible
        console.log(`Non-one-time plan detected (${eligibilityData.plan || subscriptionDetails.planName}), all resumes are eligible`);
                    ...
                    <div style={{ 
                      ...
                      {deleteConfirmId === resume.id ? (
                        ...
                        <>
                          ...
                          {hasActiveSubscription && (
                            <a 
                              href={eligibleResumeIds.includes(resume.id) ? `/api/resume/direct-download?id=${resume.id}` : '#'} 
                              onClick={(e) => {
                                if (!eligibleResumeIds.includes(resume.id)) {
                                  e.preventDefault();
                                  toast.error('You need to purchase a plan to download this resume');
                              ...
                              padding: '8px 15px',
                              borderRadius: '6px',
                                background: eligibleResumeIds.includes(resume.id) ? '#34a853' : '#f39c12',
                              color: 'white',
                              fontSize: '14px',
                              display: 'flex',
                              alignItems: 'center',
