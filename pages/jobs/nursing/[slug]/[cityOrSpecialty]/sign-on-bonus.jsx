...
export async function getServerSideProps({ params, query }) {
  ...
  // Check if first slug is a valid state
  ...
  try {
    // Try to fetch as city
    const result = await fetchCitySignOnBonusJobs(stateInfo.stateCode, cityOrSpecialty, page);

    if (!result) {
      return { notFound: true };
    }

    const seoMeta = generateCitySignOnBonusPageMetaTags(
      result.stateCode,
    ...
    console.error('Error fetching city sign-on bonus jobs:', error);
    return { notFound: true };
  }
}

export default function CitySignOnBonusPage({
  stateCode,
  stateFullName,
  city,
  citySlug,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
              RN Jobs with Sign-On Bonus in {city}
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} with sign-on bonus in {city}, {stateFullName}
                ...
                <>Browse nursing jobs with sign-on bonus in {city}, {stateFullName}.</>
              )}
            </p>
            {totalJobs > 0 && (
          ...
          {jobs.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                No Sign-On Bonus Jobs in {city}
              </h2>
              <p className="text-gray-600 mb-6">
                New positions with sign-on bonuses are added regularly. Check back soon or browse all sign-on bonus jobs in {stateFullName}.
              </p>
              <Link
              ...
              >
                Browse {stateFullName} Sign-On Bonus Jobs
              </Link>
            </div>
          )}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/sign-on-bonus?page=${currentPage - 1}`}
          ...
          {stats?.specialties && stats.specialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Sign-On Bonus RN Jobs by Specialty in {city}
                </h2>
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Offering Sign-On Bonuses in {city}
                </h2>
                <p className="text-gray-600">
                  Healthcare organizations in {city} offering sign-on bonuses for RNs
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
