...
import { fetchCityExperienceLevelJobs, fetchSoftZeroData, detectStateFromSlug } from '../../../../../../lib/services/jobPageData';
...
export async function getServerSideProps({ params, query }) {
  ...
  try {
    ...
    const result = await fetchCityExperienceLevelJobs(stateInfo.stateCode, citySlug, levelSlug, page);

    if (!result) {
      return { notFound: true };
    }

    const seoMeta = generateCityExperienceLevelPageMetaTags(
      result.stateCode,
    ...
    console.error('Error fetching city experience level jobs:', error);
    return { notFound: true };
  }
}

export default function CityExperienceLevelPage({
  city,
  citySlug,
  stateCode,
  stateFullName,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            ...
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {experienceLevel} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} in {city}, {stateCode}
                ...
                <>Browse {experienceLevel.toLowerCase()} nursing opportunities in {city}, {stateCode}.</>
              )}
            </p>
            {levelDescription && (
          ...
          {jobs.length > 0 ? (
            ...
            <>
              <SoftZeroContent
                title={`No ${experienceLevel} RN Jobs in ${cityDisplay} Right Now`}
                description={`${experienceLevel} positions in ${cityDisplay} are updated daily.`}
                alternatives={[
              ...
              {softZeroData?.otherExperienceLevels && (
                <RelatedJobsGrid
                  title={`Other Experience Levels in ${cityDisplay}`}
                  colorScheme="blue"
                  items={softZeroData.otherExperienceLevels
                    .filter(e => e.slug !== levelSlug)
                    .map(e => ({ label: e.label, href: `/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/experience/${e.slug}`, count: e.count }))}
                />
              )}
              {softZeroData?.otherCities && (
                <RelatedJobsGrid
                  title={`${experienceLevel} RN Jobs in Other ${stateFullName} Cities`}
                  colorScheme="green"
                  items={softZeroData.otherCities
                    .filter(c => c.slug !== citySlug)
                    .map(c => ({ label: c.label, href: `/jobs/nursing/${stateCode.toLowerCase()}/${c.slug}/experience/${levelSlug}`, count: c.count }))}
                />
              )}
            </>
          )}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/experience/${levelSlug}?page=${currentPage - 1}`}
            ...
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other Experience Levels in {city}
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex flex-wrap gap-4">
                  {stats.otherExperienceLevels.map((exp, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/experience/${exp.slug}`}
          ...
          {stats?.specialties && stats.specialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {experienceLevel} RN Jobs by Specialty in {city}
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <StickyJobAlertCTA specialty={experienceLevel} location={`${city}, ${stateCode}`} />
        </div>
      </div>
    </>
  );
