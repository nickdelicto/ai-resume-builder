...
export async function getServerSideProps({ params, query }) {
  const { slug, cityOrSpecialty, shift: shiftSlug } = params;
  ...
  try {
    const result = await fetchCityShiftJobs(stateInfo.stateCode, cityOrSpecialty, shiftSlug, page);
    ...
    if (shiftSlug !== result.shiftSlug) {
      ...
      return {
        redirect: {
          destination: `/jobs/nursing/${slug}/${cityOrSpecialty}/shift/${result.shiftSlug}${pageQuery}`,
          permanent: true
        }
      };
    }

    const seoMeta = generateCityShiftPageMetaTags(
      result.stateCode,
    ...
    console.error('Error fetching city shift jobs:', error);
    return { notFound: true };
  }
}

export default function CityShiftPage({
  shiftType,
  shiftSlug,
  stateCode,
  stateFullName,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
              {shiftType} RN Jobs in {cityDisplay}
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs}</strong> {shiftType} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} in {city}, {stateFullName}
                ...
                <>Browse {shiftType.toLowerCase()} nursing opportunities in {city}, {stateFullName}.</>
              )}
            </p>
            {totalJobs > 0 && (
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-100 text-blue-800 rounded-full text-sm font-semibold">
                ...
                <span>{totalJobs} {totalJobs === 1 ? 'job' : 'jobs'} available</span>
              </div>
            )}
          </div>
                ...
                <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-gray-200">
                  ...
                  <div className="space-y-3">
                    {stats.specialties.slice(0, 5).map((spec, idx) => (
                      <Link
                        key={idx}
                        href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/specialty/${spec.slug}/shift/${shiftSlug}`}
          ...
          {jobs.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                No {shiftType} RN Jobs in {city}
              </h2>
              <p className="text-gray-600 mb-6">
                New positions are added regularly. Check back soon or explore other shifts.
              </p>
              <Link
                href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}`}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/shift/${shiftSlug}?page=${currentPage - 1}`}
          ...
          {stats?.specialties && stats.specialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {shiftType} RN Jobs by Specialty in {city}
                </h2>
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Hiring {shiftType} RNs in {city}
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex flex-wrap gap-4">
                  {stats.otherShifts.map((shift, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/shift/${shift.slug}`}
