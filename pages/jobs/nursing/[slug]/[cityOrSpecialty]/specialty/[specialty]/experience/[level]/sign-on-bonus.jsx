...
export async function getServerSideProps({ params, query }) {
  ...
  try {
    const result = await fetchCitySpecialtyExperienceLevelSignOnBonusJobs(stateCode, citySlug, specialtySlug, levelSlug, page);

    if (!result) {
      return { notFound: true };
    }

    const seoMeta = generateCitySpecialtyExperienceLevelSignOnBonusPageMetaTags(
      result.stateCode,
    ...
    console.error('Error fetching city specialty experience sign-on bonus jobs:', error);
    return { notFound: true };
  }
}

export default function CitySpecialtyExperienceLevelSignOnBonusPage({
  specialty,
  specialtySlug,
  experienceLevel,
  levelSlug,
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-gray-50 py-8" style={{ fontFamily: "var(--font-figtree), 'Inter', sans-serif" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="mb-6 flex items-center gap-2 text-sm text-gray-600 flex-wrap">
            ...
            <span>/</span>
            <Link href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/specialty/${specialtySlug}/sign-on-bonus`} className="hover:text-blue-600 transition-colors">{specialty}</Link>
            <span>/</span>
          ...
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
              {experienceLevel} {specialty} RN Jobs with Sign-On Bonus in {city}
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {experienceLevel.toLowerCase()} {specialty} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} with sign-on bonus in {city}, {stateFullName}
                ...
                <>Browse {experienceLevel.toLowerCase()} {specialty} nursing jobs with sign-on bonus in {city}, {stateFullName}.</>
              )}
            </p>
            {totalJobs > 0 && (
          ...
          {jobs.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                No {experienceLevel} {specialty} Sign-On Bonus Jobs in {city}
              </h2>
              <p className="text-gray-600 mb-6">
                New positions are added regularly. Check back soon or browse other options.
              </p>
              <div className="flex flex-wrap justify-center gap-4">
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/specialty/${specialtySlug}/sign-on-bonus`}
                ...
                >
                  All {specialty} Sign-On Bonus Jobs in {city}
                </Link>
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/specialty/${specialtySlug}/experience/${levelSlug}/sign-on-bonus`}
                ...
                >
                  {experienceLevel} {specialty} Jobs in {stateFullName}
                </Link>
              </div>
            </div>
          )}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/specialty/${specialtySlug}/experience/${levelSlug}/sign-on-bonus?page=${currentPage - 1}`}
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Offering {experienceLevel} {specialty} Sign-On Bonuses in {city}
                </h2>
              </div>
