...
import { fetchCitySpecialtyJobTypeJobs } from '../../../../../../../lib/services/jobPageData';
import { generateCitySpecialtyJobTypePageMetaTags } from '../../../../../../../lib/seo/jobSEO';
import { isValidSpecialtySlug, slugToSpecialty } from '../../../../../../../lib/constants/specialties';
import { isJobType, jobTypeToSlug, jobTypeToDisplay } from '../../../../../../../lib/constants/jobTypes';
 ...
 * Example: /jobs/nursing/oh/cleveland/specialty/icu/full-time
 */
export async function getServerSideProps({ params, query }) {
  const { slug: stateCode, cityOrSpecialty: citySlug, specialty: specialtySlug, jobType: jobTypeSlug } = params;
  ...
  try {
    ...
    if (normalizedJobType && normalizedJobType !== jobTypeSlug) {
      return {
        redirect: {
          destination: `/jobs/nursing/${stateCode}/${citySlug}/specialty/${specialtySlug}/${normalizedJobType}`,
          permanent: true
        }
      };
    }

    const result = await fetchCitySpecialtyJobTypeJobs(stateCode, citySlug, specialtySlug, jobTypeSlug, page);

    if (!result) {
      return { notFound: true };
    }

    const seoMeta = generateCitySpecialtyJobTypePageMetaTags(
      result.stateCode,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {jobs.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                No {jobType} {specialty} RN Jobs in {city}
              </h2>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/job-type/${jobTypeSlug}`}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/specialty/${specialtySlug}/${jobTypeSlug}?page=${currentPage - 1}`}
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Hiring {jobType} {specialty} RNs in {city}
                </h2>
              </div>
          ...
          {stats?.otherJobTypes && stats.otherJobTypes.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other Job Types for {specialty} RNs in {city}
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.otherSpecialties && stats.otherSpecialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other {jobType} RN Specialties in {city}
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 gap-4">
                  {stats.otherSpecialties.map((spec, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/specialty/${spec.slug}/${jobTypeSlug}`}
