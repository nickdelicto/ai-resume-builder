...
import { fetchCitySpecialtyShiftJobs, fetchSoftZeroData } from '../../../../../../../../lib/services/jobPageData';
import { generateCitySpecialtyShiftPageMetaTags } from '../../../../../../../../lib/seo/jobSEO';
...
export async function getServerSideProps({ params, query }) {
  const { slug: stateCode, cityOrSpecialty: citySlug, specialty: specialtySlug, shift: shiftSlug } = params;
  const page = parseInt(query.page) || 1;

  try {
    const result = await fetchCitySpecialtyShiftJobs(stateCode, citySlug, specialtySlug, shiftSlug, page);
    ...
    if (shiftSlug !== result.shiftSlug) {
      ...
      return {
        redirect: {
          destination: `/jobs/nursing/${result.stateCode.toLowerCase()}/${citySlug}/specialty/${specialtySlug}/shift/${result.shiftSlug}${pageQuery}`,
          permanent: true
        }
      };
    }

    const seoMeta = generateCitySpecialtyShiftPageMetaTags(
      result.stateCode,
    ...
    console.error('Error fetching city specialty shift jobs:', error);
    return { notFound: true };
  }
}

export default function CitySpecialtyShiftPage({
  specialty,
  specialtySlug,
  shiftType,
  shiftSlug,
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-gray-50 py-8" style={{ fontFamily: "var(--font-figtree), 'Inter', sans-serif" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="mb-6 flex items-center gap-2 text-sm text-gray-600 flex-wrap">
            ...
            <span>/</span>
            <Link href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/${specialtySlug}`} className="hover:text-blue-600 transition-colors">{specialty}</Link>
            <span>/</span>
            <span className="text-gray-900 font-medium">{shiftType}</span>
          </nav>

          {/* Header */}
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-3">
              {specialty} {shiftType} RN Jobs in {cityDisplay}
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {specialty} {shiftType.toLowerCase()} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} in {city}, {stateCode}
                ...
                <>Browse {specialty.toLowerCase()} {shiftType.toLowerCase()} nursing opportunities in {city}, {stateCode}.</>
              )}
            </p>
            {totalJobs > 0 && (
              ...
              <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-gray-200">
                ...
                <div className="space-y-3">
                  {stats.employers.slice(0, 5).map((emp, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/employer/${emp.slug}/${specialtySlug}/shift/${shiftSlug}`}
              ...
              {stats.otherShifts && stats.otherShifts.length > 0 && (
                <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-gray-200">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Other Shifts</h3>
                  </div>
                  <div className="space-y-3">
                    {stats.otherShifts.slice(0, 5).map((s, idx) => (
                      <Link
                        key={idx}
                        href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/specialty/${specialtySlug}/shift/${s.slug}`}
                        className="flex justify-between items-center group hover:text-indigo-600 transition-colors py-1"
                      >
                        <span className="text-gray-900 group-hover:text-indigo-600 font-medium">{s.display}</span>
                        <span className="text-indigo-600 font-semibold bg-indigo-50 px-2 py-1 rounded-full text-xs">{s.count}</span>
                      </Link>
                    ))}
                  </div>
                </div>
          ...
          {jobs.length > 0 ? (
            ...
            <>
              ...
              {softZeroData?.otherShifts && (
                <RelatedJobsGrid
                  title={`Other Shifts for ${specialty} in ${cityDisplay}`}
                  colorScheme="blue"
                  items={softZeroData.otherShifts
                    .filter(s => s.slug !== shiftSlug)
                    .map(s => ({ label: s.label, href: `/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/specialty/${specialtySlug}/shift/${s.slug}`, count: s.count }))}
                />
              )}
              {softZeroData?.otherCities && (
                <RelatedJobsGrid
                  title={`${specialty} ${shiftType} RN Jobs in Other ${stateFullName} Cities`}
                  colorScheme="green"
                  items={softZeroData.otherCities
                    .filter(c => c.slug !== citySlug)
                    .map(c => ({ label: c.label, href: `/jobs/nursing/${stateCode.toLowerCase()}/${c.slug}/specialty/${specialtySlug}/shift/${shiftSlug}`, count: c.count }))}
                />
              )}
            </>
          )}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/${citySlug}/specialty/${specialtySlug}/shift/${shiftSlug}?page=${currentPage - 1}`}
            ...
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Hiring {specialty} {shiftType} RNs in {city}
                </h2>
              </div>
          ...
          {stats?.otherShifts && stats.otherShifts.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other Shift Types for {specialty} RNs in {city}
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
