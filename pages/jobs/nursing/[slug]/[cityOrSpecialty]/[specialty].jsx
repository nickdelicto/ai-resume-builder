 ...
 * This page handles City + Specialty combinations
 * Example: /jobs/nursing/ohio/cleveland/icu
 */
...
export default function CitySpecialtyPage({
  ...
  const { slug, cityOrSpecialty, specialty } = router.query || {};

  // Handle page change
  const handlePageChange = (newPage) => {
    router.push(`/jobs/nursing/${slug}/${cityOrSpecialty}/${specialty}?page=${newPage}`);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            ...
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {pagination?.total > 0 ? (
                <>
                  Find <strong>{pagination.total}</strong> <strong>{specialtyDisplayName}</strong> Registered Nurse (RN) job{pagination.total === 1 ? '' : 's'} in <strong>{cityDisplayName}, {stateDisplayName}</strong>. Browse positions at top healthcare employers
                ...
                <>Find <strong>{specialtyDisplayName}</strong> Registered Nurse (RN) positions in <strong>{cityDisplayName}, {stateDisplayName}</strong>. Browse nursing jobs at top healthcare employers. Apply today!</>
              )}
            </p>
