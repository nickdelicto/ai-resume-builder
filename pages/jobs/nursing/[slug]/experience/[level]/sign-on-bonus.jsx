...
import { fetchStateExperienceLevelSignOnBonusJobs, fetchSoftZeroData } from '../../../../../../lib/services/jobPageData';
import { generateStateExperienceLevelSignOnBonusPageMetaTags } from '../../../../../../lib/seo/jobSEO';
...
export async function getServerSideProps({ params, query }) {
  const { slug: stateCode, level: levelSlug } = params;
  const page = parseInt(query.page) || 1;

  try {
    const result = await fetchStateExperienceLevelSignOnBonusJobs(stateCode, levelSlug, page);

    if (!result) {
      return { notFound: true };
    }

    const seoMeta = generateStateExperienceLevelSignOnBonusPageMetaTags(
      result.stateCode,
    ...
    console.error('Error fetching state experience level sign-on bonus jobs:', error);
    return { notFound: true };
  }
}

export default function StateExperienceLevelSignOnBonusPage({
  experienceLevel,
  levelSlug,
  stateCode,
  stateFullName,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
              {experienceLevel} RN Jobs with Sign-On Bonus in {stateFullName}
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {experienceLevel} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} with sign-on bonus in {stateFullName}
                ...
                <>Browse {experienceLevel} nursing jobs with sign-on bonus in {stateFullName}.</>
              )}
            </p>
            {totalJobs > 0 && (
                ...
                <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-gray-200">
                  ...
                  <div className="space-y-3">
                    {stats.cities.slice(0, 5).map((c, idx) => (
                      <Link
                        key={idx}
                        href={`/jobs/nursing/${stateCode.toLowerCase()}/${c.slug}/sign-on-bonus`}
          ...
          {jobs.length > 0 ? (
            ...
            <>
              <SoftZeroContent
                title={`No ${experienceLevel} Sign-On Bonus RN Jobs in ${stateFullName} Right Now`}
                description={`${experienceLevel} sign-on bonus positions in ${stateFullName} are updated daily.`}
                alternatives={[
                  ...
                  { label: `Browse All Jobs in ${stateFullName}`, href: `/jobs/nursing/${stateCode.toLowerCase()}` },
                ]}
              />
              {softZeroData?.otherExperienceLevels && (
                <RelatedJobsGrid
                  title={`Other Experience Levels with Sign-On Bonus in ${stateFullName}`}
                  colorScheme="blue"
                  items={softZeroData.otherExperienceLevels
                    .filter(e => e.slug !== levelSlug)
                    .map(e => ({ label: e.label, href: `/jobs/nursing/${stateCode.toLowerCase()}/experience/${e.slug}/sign-on-bonus`, count: e.count }))}
                />
              )}
              {softZeroData?.otherStates && (
                <RelatedJobsGrid
                  title={`${experienceLevel} Sign-On Bonus RN Jobs in Other States`}
                  colorScheme="green"
                  items={softZeroData.otherStates
                    .filter(s => s.slug !== stateCode.toLowerCase())
                    .map(s => ({ label: s.label, href: `/jobs/nursing/${s.slug}/experience/${levelSlug}/sign-on-bonus`, count: s.count }))}
                />
              )}
            </>
          )}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/experience/${levelSlug}/sign-on-bonus?page=${currentPage - 1}`}
            ...
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {experienceLevel} Sign-On Bonus RN Jobs by City in {stateFullName}
                </h2>
                <p className="text-gray-600">
                  Find {experienceLevel} nursing jobs with sign-on bonus in {stats.cities.length} {stats.cities.length === 1 ? 'city' : 'cities'}
                </p>
              </div>
          ...
          {stats?.specialties && stats.specialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {experienceLevel} Sign-On Bonus RN Jobs by Specialty in {stateFullName}
                </h2>
                <p className="text-gray-600">
                  Explore {stats.specialties.length} nursing {stats.specialties.length === 1 ? 'specialty' : 'specialties'} with {experienceLevel.toLowerCase()} sign-on bonus positions
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 gap-4">
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Offering {experienceLevel} Sign-On Bonuses in {stateFullName}
                </h2>
                <p className="text-gray-600">
                  Leading healthcare organizations in {stateFullName} offering {experienceLevel.toLowerCase()} sign-on bonuses for RNs
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.experienceLevels && stats.experienceLevels.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other Experience Levels with Sign-On Bonus in {stateFullName}
                </h2>
                <p className="text-gray-600">
                  Browse sign-on bonus jobs for other experience levels
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex flex-wrap gap-4">
                  {stats.experienceLevels.filter(e => e.slug !== levelSlug).map((exp, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/${stateCode.toLowerCase()}/experience/${exp.slug}/sign-on-bonus`}
                      className="flex items-center gap-2 px-4 py-2 bg-teal-50 hover:bg-teal-100 rounded-lg transition-colors group"
                    >
                      <span className="text-gray-900 group-hover:text-teal-700 font-medium">{exp.level}</span>
