...
import { fetchStateSpecialtyJobTypeJobs, fetchSoftZeroData } from '../../../../../../lib/services/jobPageData';
import { generateStateSpecialtyJobTypePageMetaTags } from '../../../../../../lib/seo/jobSEO';
...
export async function getServerSideProps({ params, query }) {
  const { slug: stateCode, specialty: specialtySlug, jobType: jobTypeSlug } = params;
  const page = parseInt(query.page) || 1;

  try {
    const result = await fetchStateSpecialtyJobTypeJobs(stateCode, specialtySlug, jobTypeSlug, page);

    if (!result) {
      return { notFound: true };
    }

    // Redirect to canonical URL if job type slug doesn't match (e.g., /prn -> /per-diem)
    if (jobTypeSlug !== result.jobTypeSlug) {
      const pageQuery = page > 1 ? `?page=${page}` : '';
      return {
        redirect: {
          destination: `/jobs/nursing/${result.stateCode.toLowerCase()}/specialty/${specialtySlug}/${result.jobTypeSlug}${pageQuery}`,
          permanent: true
        }
      };
    }

    const seoMeta = generateStateSpecialtyJobTypePageMetaTags(
      result.stateCode,
    ...
    console.error('Error fetching state specialty job type jobs:', error);
    return { notFound: true };
  }
}

export default function StateSpecialtyJobTypePage({
  stateCode,
  stateFullName,
  specialty,
  specialtySlug,
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-gray-50 py-8" style={{ fontFamily: "var(--font-figtree), 'Inter', sans-serif" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-gray-200">
                  ...
                  <div className="space-y-3">
                    {stats.cities.slice(0, 5).map((c, idx) => (
                      <Link
                        key={idx}
                        href={`/jobs/nursing/${stateCode.toLowerCase()}/${generateCitySlug(c.city)}/specialty/${specialtySlug}/${jobTypeSlug}`}
          ...
          {jobs.length > 0 ? (
            ...
            <>
              <SoftZeroContent
                title={`No ${jobType} ${specialty} RN Jobs in ${stateFullName} Right Now`}
                description={`${jobType} ${specialty} positions in ${stateFullName} are updated daily.`}
                alternatives={[
              ...
              {softZeroData?.otherJobTypes && (
                <RelatedJobsGrid
                  title={`Other Job Types for ${specialty} RNs in ${stateFullName}`}
                  colorScheme="blue"
                  items={softZeroData.otherJobTypes
                    .filter(jt => jt.slug !== jobTypeSlug)
                    .map(jt => ({ label: jt.label, href: `/jobs/nursing/${stateCode.toLowerCase()}/specialty/${specialtySlug}/${jt.slug}`, count: jt.count }))}
                />
              )}
              {softZeroData?.otherSpecialties && (
                <RelatedJobsGrid
                  title={`Other ${jobType} RN Specialties in ${stateFullName}`}
                  colorScheme="purple"
                  items={softZeroData.otherSpecialties
                    .map(s => ({ label: s.label, href: `/jobs/nursing/${stateCode.toLowerCase()}/specialty/${s.slug}/${jobTypeSlug}`, count: s.count }))}
                />
              )}
              {softZeroData?.otherStates && (
                <RelatedJobsGrid
                  title={`${jobType} ${specialty} RN Jobs in Other States`}
                  colorScheme="green"
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/specialty/${specialtySlug}/${jobTypeSlug}?page=${currentPage - 1}`}
            ...
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {jobType} {specialty} RN Jobs by City in {stateFullName}
                </h2>
                <p className="text-gray-600">
                  Find {jobType.toLowerCase()} {specialty.toLowerCase()} nursing positions in {stats.cities.length} {stats.cities.length === 1 ? 'city' : 'cities'}
                </p>
              </div>
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Hiring {jobType} {specialty} RNs in {stateFullName}
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.otherJobTypes && stats.otherJobTypes.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other Job Types for {specialty} RNs in {stateFullName}
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.otherSpecialties && stats.otherSpecialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other {jobType} RN Specialties in {stateFullName}
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 gap-4">
                  {stats.otherSpecialties.map((spec, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/${stateCode.toLowerCase()}/specialty/${spec.slug}/${jobTypeSlug}`}
