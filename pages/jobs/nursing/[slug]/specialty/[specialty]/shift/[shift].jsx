...
export async function getServerSideProps({ params, query }) {
  const { slug: stateCode, specialty: specialtySlug, shift: shiftSlug } = params;
  const page = parseInt(query.page) || 1;

  try {
    const result = await fetchStateSpecialtyShiftJobs(stateCode, specialtySlug, shiftSlug, page);
    ...
    if (shiftSlug !== result.shiftSlug) {
      ...
      return {
        redirect: {
          destination: `/jobs/nursing/${result.stateCode.toLowerCase()}/specialty/${specialtySlug}/shift/${result.shiftSlug}${pageQuery}`,
          permanent: true
        }
      };
    }

    const seoMeta = generateStateSpecialtyShiftPageMetaTags(
      result.stateCode,
    ...
    console.error('Error fetching state specialty shift jobs:', error);
    return { notFound: true };
  }
}

export default function StateSpecialtyShiftPage({
  specialty,
  specialtySlug,
  shiftType,
  shiftSlug,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-3">
              {specialty} {shiftType} RN Jobs in {stateFullName}
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {specialty} {shiftType.toLowerCase()} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} in {stateFullName}
                ...
                <>Browse {specialty.toLowerCase()} {shiftType.toLowerCase()} nursing opportunities in {stateFullName}.</>
              )}
            </p>
            {totalJobs > 0 && (
                ...
                <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-gray-200">
                  ...
                  <div className="space-y-3">
                    {stats.cities.slice(0, 5).map((c, idx) => (
                      <Link
                        key={idx}
                        href={`/jobs/nursing/${stateCode.toLowerCase()}/${c.slug}/specialty/${specialtySlug}/shift/${shiftSlug}`}
          ...
          {jobs.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                No {specialty} {shiftType} RN Jobs in {stateFullName} Currently Available
              </h2>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/specialty/${specialtySlug}/shift/${shiftSlug}?page=${currentPage - 1}`}
          ...
          {stats?.cities && stats.cities.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {specialty} {shiftType} RN Jobs by City in {stateFullName}
                </h2>
                <p className="text-gray-600">
                  Find {specialty.toLowerCase()} {shiftType.toLowerCase()} nursing positions in {stats.cities.length} {stats.cities.length === 1 ? 'city' : 'cities'}
                </p>
              </div>
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Hiring {specialty} {shiftType} RNs in {stateFullName}
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.otherShifts && stats.otherShifts.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other Shift Types for {specialty} RNs in {stateFullName}
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex flex-wrap gap-4">
                  {stats.otherShifts.map((s, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/${stateCode.toLowerCase()}/specialty/${specialtySlug}/shift/${s.slug}`}
