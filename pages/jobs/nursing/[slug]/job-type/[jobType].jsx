...
export async function getServerSideProps({ params, query }) {
  const { slug: stateSlug, jobType: jobTypeSlug } = params;
  ...
  try {
    ...
    const result = await fetchStateJobTypeJobs(stateInfo.stateCode, jobTypeSlug, page);
    ...
    if (jobTypeSlug !== result.jobTypeSlug) {
      ...
      return {
        redirect: {
          destination: `/jobs/nursing/${stateSlug}/job-type/${result.jobTypeSlug}${pageQuery}`,
          permanent: true
        }
      };
    }

    const seoMeta = generateStateJobTypePageMetaTags(
      result.stateCode,
    ...
    console.error('Error fetching state job type jobs:', error);
    return { notFound: true };
  }
}

export default function StateJobTypePage({
  stateCode,
  stateFullName,
  jobType,
  jobTypeSlug,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
              {jobType} RN Jobs in {stateFullName}
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {jobType} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} in {stateFullName}
                ...
                <>Browse {jobType.toLowerCase()} nursing opportunities in {stateFullName}.</>
              )}
            </p>
            {totalJobs > 0 && (
                ...
                <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-gray-200">
                  ...
                  <div className="space-y-3">
                    {stats.cities.slice(0, 5).map((c, idx) => (
                      <Link
                        key={idx}
                        href={`/jobs/nursing/${stateCode.toLowerCase()}/${generateCitySlug(c.city)}/job-type/${jobTypeSlug}`}
          ...
          {jobs.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                No {jobType} RN Jobs Currently Available in {stateFullName}
              </h2>
              <p className="text-gray-600 mb-6">
                New positions are added regularly. Check back soon or explore other job types.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}`}
                ...
                <Link
                  href={`/jobs/nursing/job-type/${jobTypeSlug}`}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/job-type/${jobTypeSlug}?page=${currentPage - 1}`}
          ...
          {stats?.otherJobTypes && stats.otherJobTypes.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other Job Types in {stateFullName}
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex flex-wrap gap-4">
                  {stats.otherJobTypes.map((jt, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/${stateCode.toLowerCase()}/job-type/${jt.slug}`}
          ...
          {stats?.cities && stats.cities.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {jobType} RN Jobs by City in {stateFullName}
                </h2>
                <p className="text-gray-600">
                  Find {jobType.toLowerCase()} nursing positions in {stats.cities.length} {stats.cities.length === 1 ? 'city' : 'cities'}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.specialties && stats.specialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {jobType} RN Jobs by Specialty in {stateFullName}
                </h2>
                <p className="text-gray-600">
