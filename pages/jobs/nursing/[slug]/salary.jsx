...
const { detectStateFromSlug, fetchStateSalaryStats } = require('../../../../lib/services/jobPageData');
...
export async function getServerSideProps({ params }) {
  ...
  try {
    ...
    const result = await fetchStateSalaryStats(stateInfo.stateCode);
...
export default function StateSalaryPage({ 
  ...
  const location = stateFullName || stateCode;
  const jobCount = salaryStats?.jobCount || 0;

  // Generate SEO meta tags
  const seoMeta = seoUtils.generateSalaryPageMetaTags(location, 'state', salaryStats);
            ...
            {cities.length > 0 && (
              <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl shadow-lg p-6 border-2 border-blue-200">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="text-lg font-semibold text-gray-900">Browse Salary by City in {location}</h2>
