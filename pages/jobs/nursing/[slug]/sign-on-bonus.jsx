...
import { fetchStateSignOnBonusJobs, fetchSoftZeroData, detectStateFromSlug } from '../../../../lib/services/jobPageData';
import { generateStateSignOnBonusPageMetaTags } from '../../../../lib/seo/jobSEO';
...
export async function getServerSideProps({ params, query }) {
  ...
  try {
    const result = await fetchStateSignOnBonusJobs(stateInfo.stateCode, page);

    if (!result) {
      return { notFound: true };
    }

    const seoMeta = generateStateSignOnBonusPageMetaTags(
      result.stateCode,
    ...
    console.error('Error fetching state sign-on bonus jobs:', error);
    return { notFound: true };
  }
}

export default function StateSignOnBonusPage({
  stateCode,
  stateFullName,
  jobs,
  totalJobs,
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-gray-50 py-8" style={{ fontFamily: "var(--font-figtree), 'Inter', sans-serif" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="mb-6 flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>/</span>
            <span className="text-gray-900 font-medium">{stateFullName}</span>
          </nav>

          {/* Header */}
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
              RN Jobs with Sign-On Bonus in {stateFullName}
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} with sign-on bonus in {stateFullName}
                ...
                <>Browse nursing jobs with sign-on bonus in {stateFullName}.</>
              )}
            </p>
            {totalJobs > 0 && (
          ...
          {jobs.length > 0 ? (
            ...
            <>
              <SoftZeroContent
                title={`No Sign-On Bonus RN Jobs in ${stateFullName} Right Now`}
                description={`Sign-on bonus positions in ${stateFullName} are updated daily.`}
                alternatives={[
                  { label: `View All Jobs in ${stateFullName}`, href: `/jobs/nursing/${stateCode.toLowerCase()}` },
                  { label: 'View All Sign-On Bonus Jobs', href: '/jobs/nursing/sign-on-bonus' },
                  { label: 'Browse All RN Jobs', href: '/jobs/nursing' },
                ]}
              />
              {softZeroData?.specialtiesWithBonus && (
                <RelatedJobsGrid
                  title={`Specialties with Sign-On Bonus in ${stateFullName}`}
                  colorScheme="purple"
              ...
              {softZeroData?.otherStates && (
                <RelatedJobsGrid
                  title="Sign-On Bonus RN Jobs in Other States"
                  colorScheme="green"
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/${stateCode.toLowerCase()}/sign-on-bonus?page=${currentPage - 1}`}
            ...
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Sign-On Bonus RN Jobs by City in {stateFullName}
                </h2>
                <p className="text-gray-600">
                  Find nursing jobs with sign-on bonus in {stats.cities.length} {stats.cities.length === 1 ? 'city' : 'cities'}
                </p>
              </div>
          ...
          {stats?.specialties && stats.specialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Sign-On Bonus RN Jobs by Specialty in {stateFullName}
                </h2>
                <p className="text-gray-600">
                  Explore {stats.specialties.length} nursing {stats.specialties.length === 1 ? 'specialty' : 'specialties'} with sign-on bonus positions
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 gap-4">
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Offering Sign-On Bonuses in {stateFullName}
                </h2>
                <p className="text-gray-600">
                  Leading healthcare organizations in {stateFullName} offering sign-on bonuses for RNs
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
