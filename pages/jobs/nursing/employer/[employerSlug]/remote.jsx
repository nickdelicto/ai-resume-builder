...
export async function getServerSideProps({ params, query }) {
  ...
  try {
    const result = await fetchEmployerRemoteJobs(employerSlug, page);

    if (!result) {
      return { notFound: true };
    }

    const seoMeta = generateEmployerRemotePageMetaTags(
    ...
    console.error('Error fetching employer remote jobs:', error);
    return { notFound: true };
  }
}

export default function EmployerRemotePage({
  employer,
  jobs,
  totalJobs,
  totalPages,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <div className="flex items-start gap-6 mb-4">
              ...
              <div>
                ...
                <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">
                  Remote RN Jobs at {employer.name}
                </h1>
                <p className="text-lg text-gray-600">
                  {totalJobs > 0 ? (
                    <>
                      Find <strong>{totalJobs}</strong> remote Registered Nurse {totalJobs === 1 ? 'position' : 'positions'} at {employer.name}
                      {stats?.states && stats.states.length > 0 && (
                        <> requiring licenses from <strong>{stats.states.length}</strong> {stats.states.length === 1 ? 'state' : 'states'}</>
                      )}
                    ...
                    <>Browse remote nursing opportunities at {employer.name}. Work from home positions available.</>
                  )}
                </p>
              </div>
            </div>
            {totalJobs > 0 && (
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-100 text-green-800 rounded-full text-sm font-semibold">
                ...
                <span>{totalJobs} remote {totalJobs === 1 ? 'job' : 'jobs'} available</span>
              </div>
            )}
          </div>

          {/* Stats Cards */}
          {stats && (stats.states?.length > 0 || stats.specialties?.length > 0 || stats.jobTypes?.length > 0) && (
                ...
                <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-gray-200">
                  ...
                  <div className="space-y-3">
                    {stats.states.slice(0, 5).map((s, idx) => {
                      const stateFullName = getStateFullName(s.state) || s.state;
                      return (
                        <Link
                          key={idx}
                        ...
                        >
                          <span className="text-gray-900 group-hover:text-blue-600 font-medium">{stateFullName}</span>
          ...
          {jobs.length > 0 ? (
                ...
                return (
                  <React.Fragment key={job.id}>
                    ...
                    {(index + 1) % 5 === 0 && index < jobs.length - 1 && (
                      <div className="my-6">
                        <JobAlertSignup compact={true} employer={employer.name} location="Remote" />
                      </div>
                    )}
                  </React.Fragment>
                );
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                No Remote RN Jobs Currently Available at {employer.name}
              </h2>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/employer/${employer.slug}/remote?page=${currentPage - 1}`}
          ...
          {stats?.specialties && stats.specialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Remote RN Jobs at {employer.name} by Specialty
                </h2>
              </div>
          ...
          {/* Footer: License States */}
          {stats?.states && stats.states.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {employer.name} Remote Jobs by License State
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 gap-4">
                  {stats.states.map((s, idx) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-gray-900 group-hover:text-blue-600 font-medium text-sm">{stateFullName}</span>
          ...
          {/* Other Employers with Remote Jobs */}
          {stats?.otherEmployers && stats.otherEmployers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other Employers with Remote RN Jobs
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <StickyJobAlertCTA employer={employer.name} location="Remote" />
        </div>
      </div>
    </>
  );
