...
import { fetchExperienceLevelSignOnBonusJobs, fetchSoftZeroData } from '../../../../../lib/services/jobPageData';
import { generateExperienceLevelSignOnBonusPageMetaTags } from '../../../../../lib/seo/jobSEO';
import { experienceLevelToDisplay } from '../../../../../lib/constants/experienceLevels';
...
export async function getServerSideProps({ params, query }) {
  ...
  try {
    const result = await fetchExperienceLevelSignOnBonusJobs(levelSlug, page);

    if (!result) {
      return { notFound: true };
    }

    const seoMeta = generateExperienceLevelSignOnBonusPageMetaTags(
    ...
    console.error('Error fetching experience level sign-on bonus jobs:', error);
    return { notFound: true };
  }
}

export default function ExperienceLevelSignOnBonusPage({
  experienceLevel,
  levelSlug,
  jobs,
  totalJobs,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
              {experienceLevel} RN Jobs with Sign-On Bonus
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {experienceLevel} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} with sign-on bonus
                ...
                <>Browse {experienceLevel} nursing jobs with sign-on bonus nationwide.</>
              )}
            </p>
            {totalJobs > 0 && (
          ...
          {/* Stats Cards */}
          {stats && (stats.states?.length > 0 || stats.employers?.length > 0) && (
                ...
                <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-gray-200">
                  ...
                  <div className="space-y-3">
                    {stats.states.slice(0, 5).map((s, idx) => (
                      <Link
                        key={idx}
                        href={`/jobs/nursing/${s.stateCode.toLowerCase()}/experience/${levelSlug}/sign-on-bonus`}
          ...
          {jobs.length > 0 ? (
            ...
            <>
              <SoftZeroContent
                title={`No ${experienceLevel} Sign-On Bonus RN Jobs Right Now`}
                description={`${experienceLevel} nursing positions with sign-on bonuses are updated daily.`}
                alternatives={[
              ...
              {softZeroData?.otherExperienceLevels && (
                <RelatedJobsGrid
                  title="Other Experience Levels with Sign-On Bonus"
                  colorScheme="blue"
                  items={softZeroData.otherExperienceLevels
                    .filter(el => el.slug !== levelSlug)
                    .map(el => ({ label: el.label, href: `/jobs/nursing/experience/${el.slug}/sign-on-bonus`, count: el.count }))}
                />
              )}
              {softZeroData?.topStates && (
                <RelatedJobsGrid
                  title={`${experienceLevel} Sign-On Bonus RN Jobs by State`}
                  colorScheme="green"
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/experience/${levelSlug}/sign-on-bonus?page=${currentPage - 1}`}
            ...
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {experienceLevel} Sign-On Bonus RN Jobs by State
                </h2>
                <p className="text-gray-600">
                  Find {experienceLevel} nursing jobs with sign-on bonus in {stats.states.length} {stats.states.length === 1 ? 'state' : 'states'}
                </p>
              </div>
          ...
          {stats?.specialties && stats.specialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {experienceLevel} Sign-On Bonus RN Jobs by Specialty
                </h2>
                <p className="text-gray-600">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 gap-4">
                  {stats.specialties.map((spec, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/specialty/${spec.slug}/sign-on-bonus`}
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Offering {experienceLevel} Sign-On Bonuses
                </h2>
                <p className="text-gray-600">
                  Leading healthcare organizations offering {experienceLevel.toLowerCase()} sign-on bonuses for RNs
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
