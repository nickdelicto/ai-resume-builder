...
export async function getServerSideProps({ params, query }) {
  const { specialty: specialtySlug, jobType: jobTypeSlug } = params;
  const page = parseInt(query.page) || 1;

  try {
    const result = await fetchRemoteSpecialtyJobTypeJobs(specialtySlug, jobTypeSlug, page);
    ...
    if (jobTypeSlug !== result.jobTypeSlug) {
      ...
      return {
        redirect: {
          destination: `/jobs/nursing/remote/${specialtySlug}/${result.jobTypeSlug}${pageQuery}`,
          permanent: true
        }
      };
    }

    const seoMeta = generateRemoteSpecialtyJobTypePageMetaTags(
      result.specialty,
    ...
    console.error('Error fetching remote specialty job type jobs:', error);
    return { notFound: true };
  }
}

export default function RemoteSpecialtyJobTypePage({
  specialty,
  specialtySlug,
  jobType,
  jobTypeSlug,
      ...
      <div className="min-h-screen bg-gradient-to-b from-green-50 to-gray-50 py-8" style={{ fontFamily: "var(--font-figtree), 'Inter', sans-serif" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="mb-6 flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>/</span>
            <Link href={`/jobs/nursing/remote/${specialtySlug}`} className="hover:text-green-600 transition-colors">{specialty}</Link>
            <span>/</span>
          ...
          <div className="mb-8">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-3">
              {jobType} Remote {specialty} RN Jobs
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {jobType.toLowerCase()} remote {specialty} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} available
                ...
                <>Browse {jobType.toLowerCase()} remote {specialty.toLowerCase()} nursing opportunities. Work from home positions available.</>
              )}
            </p>
            {totalJobs > 0 && (
          ...
          {stats && (stats.states?.length > 0 || stats.employers?.length > 0) && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              {/* Top States Card - links to state remote page */}
              {stats.states && stats.states.length > 0 && (
                <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-gray-200">
                  ...
                  <div className="space-y-3">
                    {stats.states.slice(0, 5).map((s, idx) => (
                      <Link
                        key={idx}
                        href={`/jobs/nursing/${s.state.toLowerCase()}/remote`}
              ...
              {/* Top Employers Card - links to employer remote page */}
              {stats.employers && stats.employers.length > 0 && (
                <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 border border-gray-200">
                  <div className="flex items-center gap-2 mb-4">
                ...
                return (
                  <React.Fragment key={job.id}>
                    ...
                    {(index + 1) % 5 === 0 && index < jobs.length - 1 && (
                      <div className="my-6">
                        <JobAlertSignup compact={true} specialty={specialty} location="Remote" />
                      </div>
                    )}
                  </React.Fragment>
                );
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/remote/${specialtySlug}/${jobTypeSlug}?page=${currentPage - 1}`}
          ...
          {/* Footer: Browse Remote Jobs by State */}
          {stats?.states && stats.states.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Remote {specialty} RN Jobs by License State
                </h2>
                <p className="text-gray-600">
                  Find remote {specialty.toLowerCase()} nursing positions for nurses licensed in {stats.states.length} {stats.states.length === 1 ? 'state' : 'states'}
                </p>
              </div>
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Hiring Remote {specialty} RNs
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.otherJobTypes && stats.otherJobTypes.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other Remote Job Types for {specialty} RNs
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex flex-wrap gap-4">
                  {stats.otherJobTypes.map((jt, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/remote/${specialtySlug}/${jt.slug}`}
          ...
          <StickyJobAlertCTA specialty={specialty} location="Remote" />
        </div>
      </div>
    </>
  );
