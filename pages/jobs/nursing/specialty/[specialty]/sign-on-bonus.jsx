...
export async function getServerSideProps({ params, query }) {
  const { specialty: specialtySlug } = params;
  const page = parseInt(query.page) || 1;

  try {
    const result = await fetchSpecialtySignOnBonusJobs(specialtySlug, page);

    if (!result) {
      return { notFound: true };
    }

    const seoMeta = generateSpecialtySignOnBonusPageMetaTags(
      result.specialty,
    ...
    console.error('Error fetching specialty sign-on bonus jobs:', error);
    return { notFound: true };
  }
}

export default function SpecialtySignOnBonusPage({
  specialty,
  specialtySlug,
  jobs,
  totalJobs,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
              {specialty} RN Jobs with Sign-On Bonus
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {specialty} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} with sign-on bonus
                ...
                <>Browse {specialty.toLowerCase()} nursing jobs with sign-on bonus.</>
              )}
            </p>
            {totalJobs > 0 && (
          ...
          {jobs.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                No {specialty} Sign-On Bonus Jobs Available
              </h2>
              <p className="text-gray-600 mb-6">
                New {specialty.toLowerCase()} positions with sign-on bonuses are added regularly. Check back soon or browse all sign-on bonus jobs.
              </p>
              <Link
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/specialty/${specialtySlug}/sign-on-bonus?page=${currentPage - 1}`}
          ...
          {stats?.states && stats.states.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {specialty} Sign-On Bonus RN Jobs by State
                </h2>
                <p className="text-gray-600">
                  Find {specialty.toLowerCase()} nursing jobs with sign-on bonus in {stats.states.length} {stats.states.length === 1 ? 'state' : 'states'}
                </p>
              </div>
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Offering {specialty} Sign-On Bonuses
                </h2>
                <p className="text-gray-600">
                  Leading healthcare organizations offering sign-on bonuses for {specialty.toLowerCase()} RNs
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
