...
export async function getServerSideProps({ params, query }) {
  const { specialty: specialtySlug, level: levelSlug } = params;
  ...
  try {
    const result = await fetchSpecialtyExperienceLevelJobs(specialtySlug, levelSlug, page);

    if (!result) {
      return { notFound: true };
    }

    const seoMeta = generateSpecialtyExperienceLevelPageMetaTags(
      result.specialty,
    ...
    console.error('Error fetching specialty experience level jobs:', error);
    return { notFound: true };
  }
}

export default function SpecialtyExperienceLevelPage({
  specialty,
  specialtySlug,
  experienceLevel,
  levelSlug,
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-gray-50 py-8" style={{ fontFamily: "var(--font-figtree), 'Inter', sans-serif" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="mb-6 flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>/</span>
            <Link href={`/jobs/nursing/specialty/${specialtySlug}`} className="hover:text-blue-600 transition-colors">{specialty}</Link>
            <span>/</span>
          ...
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-3">
              {experienceLevel} {specialty} RN Jobs
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {experienceLevel.toLowerCase()} {specialty} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} available
                ...
                <>Browse {experienceLevel.toLowerCase()} {specialty.toLowerCase()} nursing opportunities across the United States.</>
              )}
            </p>
            {levelDescription && (
          ...
          {levelSlug === 'new-grad' && (
            ...
            >
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-teal-500 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/specialty/${specialtySlug}/experience/${levelSlug}?page=${currentPage - 1}`}
            ...
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {experienceLevel} {specialty} RN Jobs by State
                </h2>
                <p className="text-gray-600">
                  Find {experienceLevel.toLowerCase()} {specialty.toLowerCase()} nursing positions in {stats.states.length} {stats.states.length === 1 ? 'state' : 'states'}
                </p>
              </div>
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Hiring {experienceLevel} {specialty} RNs
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.otherExperienceLevels && stats.otherExperienceLevels.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other Experience Levels for {specialty} RNs
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex flex-wrap gap-4">
                  {stats.otherExperienceLevels.map((exp, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/specialty/${specialtySlug}/experience/${exp.slug}`}
          ...
          <StickyJobAlertCTA specialty={`${experienceLevel} ${specialty}`} />
        </div>
      </div>
    </>
  );
