...
export async function getServerSideProps({ params, query }) {
  const { specialty: specialtySlug, shift: shiftSlug } = params;
  const page = parseInt(query.page) || 1;

  try {
    const result = await fetchSpecialtyShiftJobs(specialtySlug, shiftSlug, page);
    ...
    if (shiftSlug !== result.shiftSlug) {
      ...
      return {
        redirect: {
          destination: `/jobs/nursing/specialty/${specialtySlug}/shift/${result.shiftSlug}${pageQuery}`,
          permanent: true
        }
      };
    }

    const seoMeta = generateSpecialtyShiftPageMetaTags(
      result.specialty,
    ...
    console.error('Error fetching specialty shift jobs:', error);
    return { notFound: true };
  }
}

export default function SpecialtyShiftPage({
  specialty,
  specialtySlug,
  shiftType,
  shiftSlug,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-3">
              {specialty} {shiftType} RN Jobs
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {specialty} {shiftType.toLowerCase()} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} available
                ...
                <>Browse {specialty.toLowerCase()} {shiftType.toLowerCase()} nursing opportunities across the United States.</>
              )}
            </p>
            {totalJobs > 0 && (
          ...
          {jobs.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                No {specialty} {shiftType} RN Jobs Currently Available
              </h2>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <Link
                  href={`/jobs/nursing/shift/${shiftSlug}`}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/specialty/${specialtySlug}/shift/${shiftSlug}?page=${currentPage - 1}`}
          ...
          {stats?.states && stats.states.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {specialty} {shiftType} RN Jobs by State
                </h2>
                <p className="text-gray-600">
                  Find {specialty.toLowerCase()} {shiftType.toLowerCase()} nursing positions in {stats.states.length} {stats.states.length === 1 ? 'state' : 'states'}
                </p>
              </div>
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Hiring {specialty} {shiftType} RNs
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.otherShifts && stats.otherShifts.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other Shift Types for {specialty} RNs
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex flex-wrap gap-4">
                  {stats.otherShifts.map((s, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/specialty/${specialtySlug}/shift/${s.slug}`}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex flex-wrap gap-4">
                  {stats.jobTypes.map((jt, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/specialty/${specialtySlug}/${jt.slug}`}
