...
export async function getServerSideProps({ params, query }) {
  ...
  try {
    const result = await fetchSpecialtyJobTypeJobs(specialtySlug, jobTypeSlug, page);
    ...
    if (jobTypeSlug !== result.jobTypeSlug) {
      ...
      return {
        redirect: {
          destination: `/jobs/nursing/specialty/${specialtySlug}/${result.jobTypeSlug}${pageQuery}`,
          permanent: true
        }
      };
    }

    const seoMeta = generateSpecialtyJobTypePageMetaTags(
      result.specialty,
    ...
    console.error('Error fetching specialty job type jobs:', error);
    return { notFound: true };
  }
}

export default function SpecialtyJobTypePage({
  specialty,
  specialtySlug,
  jobType,
  jobTypeSlug,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-3">
              {jobType} {specialty} RN Jobs
            </h1>
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {jobType.toLowerCase()} {specialty} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} available
                ...
                <>Browse {jobType.toLowerCase()} {specialty.toLowerCase()} nursing opportunities across the United States.</>
              )}
            </p>
            {totalJobs > 0 && (
          ...
          {jobs.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                No {jobType} {specialty} RN Jobs Currently Available
              </h2>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/specialty/${specialtySlug}/${jobTypeSlug}?page=${currentPage - 1}`}
          ...
          {stats?.states && stats.states.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {jobType} {specialty} RN Jobs by State
                </h2>
                <p className="text-gray-600">
                  Find {jobType.toLowerCase()} {specialty.toLowerCase()} nursing positions in {stats.states.length} {stats.states.length === 1 ? 'state' : 'states'}
                </p>
              </div>
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Hiring {jobType} {specialty} RNs
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.otherJobTypes && stats.otherJobTypes.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Other Job Types for {specialty} RNs
                </h2>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
