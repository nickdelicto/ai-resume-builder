...
import { fetchJobTypeJobs, fetchSoftZeroData } from '../../../../lib/services/jobPageData';
import { generateJobTypePageMetaTags } from '../../../../lib/seo/jobSEO';
...
export async function getServerSideProps({ params, query }) {
  const { jobType: jobTypeSlug } = params;
  const page = parseInt(query.page) || 1;

  try {
    const result = await fetchJobTypeJobs(jobTypeSlug, page);
    ...
    if (jobTypeSlug !== result.jobTypeSlug) {
      ...
      return {
        redirect: {
          destination: `/jobs/nursing/job-type/${result.jobTypeSlug}${pageQuery}`,
          permanent: true
        }
      };
    }

    const seoMeta = generateJobTypePageMetaTags(
      result.jobType,
      result.jobTypeSlug,
      result.totalJobs,
    ...
    console.error('Error fetching job type jobs:', error);
    return { notFound: true };
  }
}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            ...
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {jobType} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} available
                ...
                <>Browse {jobType.toLowerCase()} nursing opportunities across the United States.</>
              )}
            </p>
            {totalJobs > 0 && (
          ...
          {jobs.length > 0 ? (
            ...
            <>
              <SoftZeroContent
                title={`No ${jobType} RN Jobs Right Now`}
                description={`${jobType} nursing positions are updated daily.`}
                alternatives={[
                  ...[
                  ...
                  ].filter(jt => jt.slug !== jobTypeSlug).slice(0, 2).map(jt => ({
                    label: `View ${jt.label} RN Jobs`,
                    href: `/jobs/nursing/job-type/${jt.slug}`
                  })),
              ...
              {softZeroData?.otherJobTypes && (
                <RelatedJobsGrid
                  title="Other Job Types Hiring"
                  colorScheme="blue"
                  items={softZeroData.otherJobTypes
                    .filter(jt => jt.slug !== jobTypeSlug)
                    .map(jt => ({ label: jt.label, href: `/jobs/nursing/job-type/${jt.slug}`, count: jt.count }))}
                />
              )}
              {softZeroData?.topStates && (
                <RelatedJobsGrid
                  title={`${jobType} RN Jobs by State`}
                  colorScheme="green"
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/job-type/${jobTypeSlug}?page=${currentPage - 1}`}
            ...
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {jobType} RN Jobs by State
                </h2>
                <p className="text-gray-600">
                  Find {jobType.toLowerCase()} nursing positions in {stats.states.length} {stats.states.length === 1 ? 'state' : 'states'}
                </p>
              </div>
          ...
          {stats?.specialties && stats.specialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {jobType} RN Jobs by Specialty
                </h2>
                <p className="text-gray-600">
                  Explore {stats.specialties.length} nursing {stats.specialties.length === 1 ? 'specialty' : 'specialties'} with {jobType.toLowerCase()} positions
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 gap-4">
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Hiring {jobType} RNs
                </h2>
                <p className="text-gray-600">
                  Leading healthcare organizations with {jobType.toLowerCase()} nursing opportunities
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.otherJobTypes && stats.otherJobTypes.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Browse Other Job Types
                </h2>
                <p className="text-gray-600">
                  Explore other nursing job types
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
