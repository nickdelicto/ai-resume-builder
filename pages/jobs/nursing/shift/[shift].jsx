...
export async function getServerSideProps({ params, query }) {
  const { shift: shiftSlug } = params;
  const page = parseInt(query.page) || 1;

  try {
    const result = await fetchShiftJobs(shiftSlug, page);
    ...
    if (shiftSlug !== result.shiftSlug) {
      ...
      return {
        redirect: {
          destination: `/jobs/nursing/shift/${result.shiftSlug}${pageQuery}`,
          permanent: true
        }
      };
    }

    const seoMeta = generateShiftPageMetaTags(
      result.shiftType,
      result.shiftSlug,
      result.totalJobs,
    ...
    console.error('Error fetching shift jobs:', error);
    return { notFound: true };
  }
}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            ...
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> {shiftType} Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} available
                ...
                <>Browse {shiftType.toLowerCase()} nursing opportunities across the United States.</>
              )}
            </p>
            {totalJobs > 0 && (
          ...
          {jobs.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                No {shiftType} RN Jobs Currently Available
              </h2>
              <p className="text-gray-600 mb-6">
                New positions are added regularly. Check back soon or explore other shift types.
              </p>
              <Link
                href="/jobs/nursing"
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/shift/${shiftSlug}?page=${currentPage - 1}`}
          ...
          {stats?.states && stats.states.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {shiftType} RN Jobs by State
                </h2>
                <p className="text-gray-600">
                  Find {shiftType.toLowerCase()} nursing positions in {stats.states.length} {stats.states.length === 1 ? 'state' : 'states'}
                </p>
              </div>
          ...
          {stats?.specialties && stats.specialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {shiftType} RN Jobs by Specialty
                </h2>
                <p className="text-gray-600">
                  Explore {stats.specialties.length} nursing {stats.specialties.length === 1 ? 'specialty' : 'specialties'} with {shiftType.toLowerCase()} positions
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 gap-4">
                  {stats.specialties.map((spec, idx) => (
                    <Link
                      key={idx}
                      href={`/jobs/nursing/specialty/${spec.slug}/shift/${shiftSlug}`}
          ...
          {/* Footer: Browse by Job Type */}
          {stats?.jobTypes && stats.jobTypes.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {shiftType} RN Jobs by Job Type
                </h2>
                <p className="text-gray-600">
                  Explore {stats.jobTypes.length} job {stats.jobTypes.length === 1 ? 'type' : 'types'} with {shiftType.toLowerCase()} schedules
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Hiring {shiftType} RNs
                </h2>
                <p className="text-gray-600">
                  Leading healthcare organizations with {shiftType.toLowerCase()} nursing opportunities
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          {stats?.otherShifts && stats.otherShifts.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Browse Other Shift Types
                </h2>
                <p className="text-gray-600">
                  Explore other shift schedules
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex flex-wrap gap-4">
