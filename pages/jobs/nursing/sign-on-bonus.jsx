...
export async function getServerSideProps({ query }) {
  ...
  try {
    const result = await fetchSignOnBonusJobs(page);

    if (!result) {
      return { notFound: true };
    }

    const seoMeta = generateSignOnBonusPageMetaTags(
      result.totalJobs,
    ...
    console.error('Error fetching sign-on bonus jobs:', error);
    return { notFound: true };
  }
}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            ...
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed mb-4">
              {totalJobs > 0 ? (
                <>
                  Find <strong>{totalJobs.toLocaleString()}</strong> Registered Nurse {totalJobs === 1 ? 'job' : 'jobs'} with sign-on bonus
                ...
                <>Browse nursing jobs with sign-on bonus across the United States.</>
              )}
            </p>
            {totalJobs > 0 && (
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 my-8">
              {currentPage > 1 && (
                <Link
                  href={`/jobs/nursing/sign-on-bonus?page=${currentPage - 1}`}
            ...
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Sign-On Bonus RN Jobs by State
                </h2>
                <p className="text-gray-600">
                  Find nursing jobs with sign-on bonus in {stats.states.length} {stats.states.length === 1 ? 'state' : 'states'}
                </p>
              </div>
          ...
          {stats?.specialties && stats.specialties.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Sign-On Bonus RN Jobs by Specialty
                </h2>
                <p className="text-gray-600">
          ...
          {stats?.employers && stats.employers.length > 0 && (
            <div className="mt-16 pt-8 border-t border-gray-200">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  Top Employers Offering Sign-On Bonuses
                </h2>
                <p className="text-gray-600">
                  Leading healthcare organizations offering sign-on bonuses for RNs
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
