    ...
    <div style={{ width: '100%', height: 250 }}>
      <ResponsiveContainer>
        <LineChart data={data}>
          ...
          <Tooltip />
          <Line yAxisId="left" type="monotone" dataKey="clicks" stroke="#ea580c" strokeWidth={2} dot={{ r: 3 }} name="Clicks" />
...
function MarkdownContent({ text, accentColor = '#ea580c' }) {
  if (!text) return null;
  return (
    <ReactMarkdown
...
export default function BingMonitor() {
  ...
  // Trend chart period (number of weeks to show)
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/admin/bing?weeks=52');
      if (!response.ok) throw new Error('Failed to fetch Bing data');
  ...
  const sendChatMessage = async (messageText) => {
    ...
    setChatLoading(true);
    try {
      const response = await fetch('/api/admin/bing-chat', {
        method: 'POST',
  ...
  // Filter chart data by selected period (weeks)
  ...
  return (
    <>
      <Head>
        <title>Bing Search Console | IntelliResume Admin</title>
      </Head>
        ...
        <div className="container mx-auto px-4 pt-24 pb-6 max-w-6xl">
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center gap-3">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Bing Search Performance</h1>
              <span className="px-2.5 py-1 bg-orange-100 text-orange-700 text-xs font-semibold rounded-full uppercase">Bing</span>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              {data.aiSummary && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                  <div className="bg-gradient-to-r from-gray-900 to-gray-800 px-6 py-3 flex items-center gap-2">
                    ...
                    <h2 className="text-white font-semibold text-sm">AI Analysis — Week of {formatDate(latest.date)}</h2>
                  </div>
                  <div className="px-6 py-5" style={{ fontFamily: 'var(--font-figtree), ui-sans-serif, system-ui' }}>
                    <div className="text-[15px] leading-7 text-gray-800">
                      <MarkdownContent text={data.aiSummary} accentColor="#ea580c" />
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                  ...
                  <div className="flex items-baseline gap-2">
                    <p className="text-2xl font-bold text-orange-600">{latest.avgPosition.toFixed(1)}</p>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex bg-gray-100 rounded-lg p-0.5">
                      {chartPeriods.map(p => (
                        <button
                          key={p.weeks}
                          onClick={() => setChartPeriod(p.weeks)}
                  ...
                  <div className="flex justify-center gap-6 mt-2">
                    <span className="flex items-center gap-1.5 text-xs text-gray-500">
                      <span className="w-3 h-0.5 bg-orange-600 rounded"></span> Clicks
                    </span>
                    <span className="flex items-center gap-1.5 text-xs text-gray-500">
                      ...
                      {data.winners.map((w, i) => (
                        <div key={i} className="flex justify-between items-center text-sm">
                          <a href={w.page} target="_blank" rel="noopener noreferrer"
                            className="text-orange-600 hover:text-orange-800 hover:underline truncate pr-2 flex-1" title={w.page}>
                      ...
                      {data.losers.map((l, i) => (
                        <div key={i} className="flex justify-between items-center text-sm">
                          <a href={l.page} target="_blank" rel="noopener noreferrer"
                            className="text-orange-600 hover:text-orange-800 hover:underline truncate pr-2 flex-1" title={l.page}>
                    ...
                    <div className="flex gap-1">
                      {[['clicks', 'Clicks'], ['impressions', 'Impr'], ['position', 'Pos']].map(([key, label]) => (
                        <button key={key} onClick={() => { setPagesSort(key); setPagesPage(0); }}
                          className={`text-xs px-2 py-0.5 rounded ${pagesSort === key ? 'bg-orange-100 text-orange-700 font-medium' : 'text-gray-400 hover:text-gray-600'}`}>
                          {label}{pagesSort === key ? ' ↓' : ''}
                        </button>
                      ))}
                    </div>
                    ...
                    {sortedPages.slice(pagesPage * PAGE_SIZE, (pagesPage + 1) * PAGE_SIZE).map((p, i) => (
                      <div key={i} className="flex justify-between items-center text-sm py-1 border-b border-gray-50 last:border-0">
                        <a href={p.page} target="_blank" rel="noopener noreferrer"
                          className="text-orange-600 hover:text-orange-800 hover:underline truncate pr-2 flex-1" title={p.page}>
                          <span className="text-gray-400 mr-1">{pagesPage * PAGE_SIZE + i + 1}.</span>
                          {p.page.length > 55 ? p.page.slice(0, 55) + '...' : p.page}
                        </a>
                        <div className="flex gap-3 text-xs flex-shrink-0">
                          <span className={`${pagesSort === 'clicks' ? 'font-semibold' : ''} text-orange-600`}>{p.clicks}c</span>
                  ...
                  {sortedPages.length > PAGE_SIZE && (
                    <div className="flex justify-between items-center mt-3 pt-2 border-t border-gray-100">
                      <button onClick={() => setPagesPage(p => Math.max(0, p - 1))} disabled={pagesPage === 0}
                        className="text-xs text-orange-600 hover:text-orange-800 disabled:text-gray-300 disabled:cursor-not-allowed">
                        Prev
                      </button>
                  ...
                  <div className="space-y-1.5">
                    {sortedQueries.slice(queriesPage * PAGE_SIZE, (queriesPage + 1) * PAGE_SIZE).map((q, i) => (
                      <div key={i} className="flex justify-between items-center text-sm py-1 border-b border-gray-50 last:border-0">
                        ...
                        <div className="flex gap-3 text-xs flex-shrink-0">
                          <span className={`${queriesSort === 'clicks' ? 'font-semibold' : ''} text-orange-600`}>{q.clicks}c</span>
              ...
              <div ref={chatSectionRef} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="px-4 py-3 border-b bg-gray-50 flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-gray-900">Ask about your Bing data</h3>
                </div>
                  ...
                  {chatMessages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[85%] rounded-xl px-4 py-2.5 text-sm leading-relaxed ${
                        ...
                        <div>
                          <MarkdownContent text={msg.content} accentColor={msg.role === 'user' ? '#ffffff' : '#ea580c'} />
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                {chatMessages.length === 0 && (
                  <div className="px-4 pb-3 flex flex-wrap gap-2">
                    {[
                      "How is Bing traffic trending?",
                      "Which pages get the most Bing clicks?",
                      "Any pages losing Bing rankings?",
                ...
                <div className="p-4 border-t bg-gray-50">
                  <textarea
                    ...
                    placeholder="Ask about your Bing search performance... (Enter to send, Shift+Enter for new line)"
                    rows={3}
                    className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent resize-none"
                  ...
                  <div className="flex justify-end mt-2">
                    <button
                      ...
                      className="px-5 py-2 bg-orange-600 text-white rounded-xl text-sm font-medium hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
                      Send
                    </button>
                  </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No Bing data yet</h3>
              <p className="text-gray-500 text-sm">Run the data pull script to populate Bing Webmaster data:</p>
              <code className="block mt-2 text-sm text-orange-600 bg-orange-50 px-4 py-2 rounded-lg">
                node scripts/pull-bing-data.js
              </code>
            </div>
          )}
        </div>
        ...
        <div className="px-4 pb-4 space-y-2">
          {pagedAlerts.map((alert, i) => {
            ...
            const chatQuestion = alert.entity
              ? `Analyze this Bing alert in detail: "${alert.title}". Which pages and keywords contributed to this? The affected entity is: ${alert.entity}`
              : `Analyze this Bing alert in detail: "${alert.title} — ${alert.description}". Which specific pages and keywords contributed to this change? Give me the numbers.`;
            ...
            return (
              <div key={i} className={`${c.bg} border ${c.border} rounded-lg px-3 py-2 flex items-start gap-2`}>
                ...
                <button
                  ...
                  className="text-xs text-gray-500 hover:text-orange-600 font-medium flex-shrink-0 px-2 py-0.5 rounded hover:bg-white/60 transition-colors"
